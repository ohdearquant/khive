...
A research knowledge graph runtime — 81 verbs, 11 packs, one MCP tool.
...
Git provenance ingestion (`git.digest`) and the hardened write verbs (`git.commit` /
`git.branch` / `git.push`, ADR-108) are provided by a commercially licensed extension and
