...
| `crates/khive-pack-moodboard`   | Opt-in experimental visual-asset ingest and exact descriptor-space retrieval using BlobStore, Lattice, and named Khive vectors ([ADR-148](docs/adr/ADR-148-moodboard-visual-retrieval-pack.md)); not in the default pack set                                  |
...
| `db_diagnostics` | —                                                                                                                   | Writer-contention and WAL/checkpoint diagnostics: aggregate plus pooled/standalone/writer-task acquisition counters, pooled checkout timeouts, audit failures, build identity, checkpoint counters, PASSIVE checkpoint probe, WAL file size, and explicitly qualified WAL-pin census. The probe may backfill WAL frames (normal checkpoint I/O); it never changes logical state, increments write-traffic acquisitions, escalates to TRUNCATE, or deletes sidecar evidence |
