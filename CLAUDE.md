...
| `db_diagnostics` | —                                                                                                                   | Writer-contention and WAL/checkpoint diagnostics: pool checkout/audit-failure counters, build identity, checkpoint counters, PASSIVE checkpoint probe, WAL file size, and explicitly qualified WAL-pin census. The probe may backfill WAL frames (normal checkpoint I/O); it never changes logical state, escalates to TRUNCATE, or deletes sidecar evidence |
