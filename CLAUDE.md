...
| `db_diagnostics` | —                                                                                                                   | Writer-contention, graph-edge integrity, and WAL/checkpoint diagnostics: aggregate plus pooled/standalone/writer-task acquisition counters, pooled checkout timeouts, audit failures, build identity, duplicate edge-ID and list-ledger counts, checkpoint counters, PASSIVE checkpoint probe, WAL file size, and explicitly qualified WAL-pin census. The probe may backfill WAL frames (normal checkpoint I/O); it never changes logical state, increments write-traffic acquisitions, escalates to TRUNCATE, or deletes sidecar evidence |
