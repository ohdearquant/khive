...
class ManifestTests(unittest.TestCase):
    ...
    def test_mcp_operation_allowlist_matches_shipped_kg_handlers(self):
        ...
        bare_verbs = {
            op
            for op in coverage_validator.AVAILABLE_OPERATIONS_BY_SURFACE["mcp_daemon"]
            if "." not in op
        }
        self.assertEqual(bare_verbs, handler_names)
    ...
    def test_unknown_admin_cli_operation_is_flagged(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = self._write_manifest(
                pathlib.Path(tmp),
                [
                    _base_scenario(
                        scenario_id="f6.nonexistent.cli.production",
                        feature="F6",
                        surface="admin_cli",
                        operation="kkernel nonexistent-op",
                    )
                ],
            )
            _, errors = coverage_validator.load_manifest(path)
