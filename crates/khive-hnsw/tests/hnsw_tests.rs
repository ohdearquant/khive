...
mod search_context_tests {
    ...
    // =========================================================================
    // build_batch (parallel HNSW construction)
    // =========================================================================
    ...
    #[test]
    fn test_build_batch_single_item() {
        let mut index = HnswIndex::new(3);
        let id = make_id(1);
        index
            .build_batch(vec![(id, vec![1.0, 0.0, 0.0])])
    ...
    #[test]
    fn test_build_batch_small_falls_back_to_sequential() {
        // <= 32 items should use sequential fallback
        let dim = 8;
        let mut index = HnswIndex::new(dim);
        let items: Vec<_> = (0..20u8)
            .map(|i| (make_id(i), generate_random_vector(dim, i as u64)))
            .collect();

        index.build_batch(items).expect("small batch");
        assert_eq!(index.len(), 20);

        // Verify search works
        let query = generate_random_vector(dim, 999);
        let results = index.search(&query, 5).expect("search");
        assert_eq!(results.len(), 5);
    }

    #[test]
    fn test_build_batch_parallel_correctness() {
        // Large enough to trigger parallel path (> 32)
        let dim = 16;
        let n = 200;
        ...
        let items: Vec<_> = (0..n)
            .map(|i| {
                ...
                (id, generate_random_vector(dim, i as u64))
            })
            .collect();

        let mut index = HnswIndex::with_config(config);
        index.build_batch(items.clone()).expect("parallel batch");

        assert_eq!(index.len(), n);

        // Nearly all IDs should find themselves as nearest neighbor.
        // The batch path searches a frozen seed graph, so a small number of
        // vectors may not find themselves as #1 due to the approximate nature
        // of the parallel build. Require >= 95% self-recall.
        let mut self_found = 0usize;
        for (id, vec) in &items {
            let results = index.search(vec, 1).expect("search");
            assert!(!results.is_empty(), "should find at least 1 result");
            if results[0].0 == *id {
                self_found += 1;
            }
        }
        let self_recall = self_found as f64 / n as f64;
        assert!(
            self_recall >= 0.95,
            "Self-recall too low: {:.1}% ({}/{}) (expected >= 95%)",
            self_recall * 100.0,
            self_found,
            n
        );
    ...
    #[test]
    fn test_build_batch_recall_quality() {
        // Compare recall: build_batch vs sequential insert
        let dim = 32;
        let n = 500;
        ...
        // Build with batch
        let config_batch = HnswConfig {
            dimensions: dim,
            seed: Some(42),
            ..HnswConfig::default()
        };
        let mut batch_index = HnswIndex::with_config(config_batch);
        batch_index.build_batch(items.clone()).expect("batch build");

        // Build sequentially
        let config_seq = HnswConfig {
            dimensions: dim,
            seed: Some(42),
            ..HnswConfig::default()
        };
        let mut seq_index = HnswIndex::with_config(config_seq);
        for (id, vec) in &items {
            seq_index
                .insert(*id, vec.clone())
        ...
        // Compare recall on random queries
        let k = 10;
        let num_queries = 20;
        ...
        for q in 0..num_queries {
            let query = generate_random_vector(dim, 10_000 + q as u64);

            let batch_results = batch_index.search(&query, k).expect("batch search");
            let seq_results = seq_index.search(&query, k).expect("seq search");

            let batch_ids: HashSet<_> = batch_results.iter().map(|(id, _)| *id).collect();
            let seq_ids: HashSet<_> = seq_results.iter().map(|(id, _)| *id).collect();

            total_overlap += batch_ids.intersection(&seq_ids).count();
        }

        // Expect at least 55% overlap between batch and sequential builds.
        // The parallel build searches a frozen seed graph with the correct degree
        // cap (m_max0 for layer 0, m for upper layers), so graph topology differs
        // from sequential builds. 55% is conservative for this comparison.
        let recall_overlap = total_overlap as f64 / total_possible as f64;
        assert!(
            recall_overlap >= 0.55,
            "Recall overlap too low: {:.1}% (expected >= 55%)",
            recall_overlap * 100.0
        );
    }

    #[test]
    fn test_build_batch_dimension_mismatch() {
        let mut index = HnswIndex::new(3);
        let result = index.build_batch(vec![(make_id(1), vec![1.0, 0.0])]);
        assert!(result.is_err());
    }

    #[test]
    fn test_build_batch_rejects_duplicate_ids() {
        let mut index = HnswIndex::new(3);
        index
            .insert(make_id(1), vec![1.0, 0.0, 0.0])
            .expect("pre-insert");
    ...
    #[test]
    fn test_build_batch_search_after_build() {
        // Verify that search still works correctly after batch build
        let dim = 16;
        let n = 100;
        ...
        index.build_batch(items.clone()).expect("batch build");

        // Search for the first vector -- should find itself
        let results = index.search(&items[0].1, 1).expect("search");
        assert_eq!(results[0].0, items[0].0);
        assert!(
            results[0].1.to_f64() > 0.99,
            "exact match should have high score"
        );
    }

    #[test]
    fn test_build_batch_with_delete_after() {
        // Build batch, then delete, then search
        let dim = 8;
        let n = 50;
        ...
        let query = generate_random_vector(dim, 999);
        let results = index.search(&query, 20).expect("search");

        let deleted_ids: HashSet<_> = items[..10].iter().map(|(id, _)| *id).collect();
        for (id, _) in &results {
            assert!(
