...
Condition validation is binding-aware. A direct node or observation-target field must name an allowed dedicated column; arbitrary and nested JSON data requires an explicit `properties.<path>` reference. The bare `properties` container, unknown direct fields, and JSON paths on bindings without a properties object are rejected. Event predicates accept only event columns, while edge predicates accept `relation` and `weight`; rejected names report the valid list. `relation` values on edge variables are taxonomy-checked. Operators that require a particular value shape, including list-valued `IN` and operand-free `IS NOT NULL`, are checked before compilation.
