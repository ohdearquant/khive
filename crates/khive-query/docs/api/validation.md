...
Condition validation is binding-aware. Node and observation-target predicates resolve dedicated columns directly and other property names through their JSON `properties` column. Event predicates accept only event columns, while edge predicates accept `relation` and `weight`; rejected names report the valid list. `relation` values on edge variables are taxonomy-checked. Operators that require a particular value shape, including list-valued `IN` and operand-free `IS NOT NULL`, are checked before compilation.
