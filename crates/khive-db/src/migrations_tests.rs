...
const LEGACY_TOOL_GRANTS_DDL: &str = "CREATE TABLE tool_grants (
    id TEXT PRIMARY KEY,
...
fn insert_legacy_tool_grants(conn: &Connection) {
        ...
        .enumerate()
    {
        conn.execute(
            "INSERT INTO tool_grants \
             (id, namespace, actor, tool, scope, reason, status, requested_at, \
              decided_at, decided_by, expires_at, decision_note) \
             VALUES (?1, ?2, 'lambda:test', 'tool:test', 'one-run', 'reason', ?3, \
                     100, 200, 'lambda:reviewer', 300, 'decision')",
            rusqlite::params![format!("grant-{i}"), format!("namespace-{i}"), status],
        )
        .unwrap();
    }
}

fn tool_grant_rows(conn: &Connection) -> Vec<Vec<rusqlite::types::Value>> {
    let mut statement = conn
        .prepare("SELECT * FROM tool_grants ORDER BY id")
        .unwrap();
...
#[test]
fn v34_tool_grant_pins_preserve_legacy_decisions_across_reopen() {
    let directory = tempfile::tempdir().unwrap();
    let path = directory.path().join("legacy-tool-grants.db");
    let expected = {
        let mut conn = Connection::open(&path).unwrap();
        migrate_through(&mut conn, 33);
        conn.execute_batch(LEGACY_TOOL_GRANTS_DDL).unwrap();
        insert_legacy_tool_grants(&conn);
        let expected: Vec<_> = tool_grant_rows(&conn)
            .into_iter()
            .map(|mut row| {
                row.extend([const { rusqlite::types::Value::Null }; 4]);
                row
            })
            .collect();

        assert_eq!(run_migrations(&mut conn).unwrap(), latest_schema_version());
        assert_eq!(tool_grant_rows(&conn), expected);
        assert!(schema_object_exists(&conn, "index", "idx_tool_grants_lookup").unwrap());
        expected
    };

    let mut reopened = Connection::open(&path).unwrap();
    let changes_before = reopened.total_changes();
    assert_eq!(
        run_migrations(&mut reopened).unwrap(),
        latest_schema_version()
    );
    assert_eq!(reopened.total_changes(), changes_before);
    assert_eq!(tool_grant_rows(&reopened), expected);
    assert_eq!(
        reopened
            .query_row(
                "SELECT COUNT(*) FROM _schema_migrations \
                 WHERE version = 34 AND name = 'tool_grant_pins'",
                [],
                |row| row.get::<_, u32>(0),
            )
            .unwrap(),
...
#[test]
fn v34_tool_grant_pins_migrate_without_a_previously_loaded_tool_pack() {
    let mut conn = open_memory();
    migrate_through(&mut conn, 33);
    assert!(!table_exists(&conn, "tool_grants"));
    assert_eq!(run_migrations(&mut conn).unwrap(), latest_schema_version());

    let mut statement = conn.prepare("PRAGMA table_info(tool_grants)").unwrap();
    ...
    assert_eq!(
        &columns[12..],
        &[
            ("registry_id".into(), "TEXT".into(), false, None, 0),
            ("definition_digest".into(), "TEXT".into(), false, None, 0),
            (
                "invalidated_by_registry_id".into(),
                "TEXT".into(),
                false,
                None,
                0,
            ),
            ("invalidated_at".into(), "INTEGER".into(), false, None, 0),
        ]
    );
    assert!(tool_grant_rows(&conn).is_empty());
    insert_legacy_tool_grants(&conn);
    for row in tool_grant_rows(&conn) {
        assert_eq!(&row[12..], &[const { rusqlite::types::Value::Null }; 4]);
    }
}

#[test]
fn v34_tool_grant_pins_accept_compatible_direct_bootstrap_without_rewriting_rows() {
    let mut conn = open_memory();
    migrate_through(&mut conn, 33);
    conn.execute_batch(V34_UP).unwrap();
    conn.execute_batch(TOOL_GRANT_INVALIDATION_UP).unwrap();
    insert_legacy_tool_grants(&conn);
    conn.execute(
        "UPDATE tool_grants SET registry_id = 'approved-registry-id', \
         definition_digest = 'approved-definition-digest' WHERE status = 'granted'",
        [],
    )
    .unwrap();
    let expected = tool_grant_rows(&conn);
    ...
    assert_eq!(tool_grant_rows(&conn), expected);
    assert_eq!(
        conn.query_row("PRAGMA schema_version", [], |row| row.get::<_, u32>(0))
            .unwrap(),
        schema_version
    );
}
...
#[test]
fn v34_tool_grant_pins_reject_partial_or_incompatible_bootstrap_without_changes() {
    for incompatible_pins in [
        ...
        migrate_through(&mut conn, 33);
        conn.execute_batch(LEGACY_TOOL_GRANTS_DDL).unwrap();
        conn.execute_batch(incompatible_pins).unwrap();
        let insert = if column_exists(&conn, "tool_grants", "registry_id") {
            "INSERT INTO tool_grants \
             (id, namespace, actor, tool, status, requested_at, registry_id) \
             VALUES ('legacy', 'local', 'lambda:test', 'tool:test', 'granted', 100, 'unapproved')"
        } else {
            "INSERT INTO tool_grants \
             (id, namespace, actor, tool, status, requested_at) \
             VALUES ('legacy', 'local', 'lambda:test', 'tool:test', 'granted', 100)"
        };
        ...
        assert!(matches!(error, SqliteError::Migration { version: 34, .. }));
        assert_eq!(read_schema_version(&conn).unwrap(), 33);
...
#[test]
fn v34_tool_grant_pins_roll_back_columns_when_ledger_recording_fails() {
    let mut conn = open_memory();
    migrate_through(&mut conn, 33);
    conn.execute_batch(LEGACY_TOOL_GRANTS_DDL).unwrap();
    insert_legacy_tool_grants(&conn);
    let expected = tool_grant_rows(&conn);
    conn.execute_batch(
        "CREATE TRIGGER fail_v34_ledger BEFORE INSERT ON _schema_migrations \
         WHEN NEW.version = 34 BEGIN SELECT RAISE(ABORT, 'ledger unavailable'); END;",
    )
    .unwrap();
    ...
    assert!(matches!(error, SqliteError::Migration { version: 34, .. }));
    assert_eq!(read_schema_version(&conn).unwrap(), 33);
    assert!(!column_exists(&conn, "tool_grants", "registry_id"));
    assert!(!column_exists(&conn, "tool_grants", "definition_digest"));
    assert!(!column_exists(
        &conn,
        "tool_grants",
        "invalidated_by_registry_id"
    ));
    assert!(!column_exists(&conn, "tool_grants", "invalidated_at"));
    ...
    conn.execute_batch("DROP TRIGGER fail_v34_ledger").unwrap();
    assert_eq!(run_migrations(&mut conn).unwrap(), latest_schema_version());
}

#[test]
fn v34_tool_grant_pins_backfill_invalidation_without_approving_legacy_rows() {
    let mut conn = open_memory();
    migrate_through(&mut conn, 33);
    conn.execute_batch(LEGACY_TOOL_GRANTS_DDL).unwrap();
    for (id, tool, status, namespace) in [
        ...
        conn.execute(
            "INSERT INTO tool_grants (id, namespace, actor, tool, status, requested_at) \
             VALUES (?1, ?2, 'lambda:test', ?3, ?4, 1)",
            rusqlite::params![id, namespace, tool, status],
        )
        .unwrap();
    }
    let original = tool_grant_rows(&conn);
    for (id, name, namespace, created_at, deleted_at, kind, tags) in [
        ...
        conn.execute(
            ...
            rusqlite::params![id, namespace, kind, name, tags, created_at, deleted_at],
        )
        .unwrap();
    }

    assert_eq!(run_migrations(&mut conn).unwrap(), latest_schema_version());
    let migrated = tool_grant_rows(&conn);
    for (before, after) in original.iter().zip(&migrated) {
        assert_eq!(&after[..12], before);
        assert_eq!(&after[12..14], &[const { rusqlite::types::Value::Null }; 2]);
    }
    let markers = |conn: &Connection| {
        conn.prepare(
            "SELECT id, invalidated_by_registry_id, invalidated_at \
    ...
    assert_eq!(
        markers(&conn),
        vec![
            ("all".into(), Some("a".into()), Some(100)),
            ("case-prefix".into(), None, None),
            ("exact".into(), Some("exact-id".into()), Some(200)),
            ("foreign".into(), None, None),
            ("future".into(), None, None),
            ("prefix".into(), Some("a".into()), Some(100)),
            ("requested".into(), None, None),
        ]
    );
    ...
    conn.execute(
        "INSERT INTO entities (id, namespace, kind, name, tags, created_at, updated_at) \
         VALUES ('future-id', 'local', 'project', 'new:tool', '[\"tool-registry\"]', 500, 500)",
        [],
    )
    .unwrap();
    conn.execute("DELETE FROM entities WHERE id = 'future-id'", [])
        .unwrap();
    let expected = markers(&conn);
    assert_eq!(
        expected[4],
        ("future".into(), Some("future-id".into()), Some(500))
    );
    assert_eq!(expected[0], ("all".into(), Some("a".into()), Some(100)));
    assert_eq!(
        expected[2],
        ("exact".into(), Some("exact-id".into()), Some(200))
    );
    conn.execute_batch(TOOL_GRANT_INVALIDATION_UP).unwrap();
    assert_eq!(markers(&conn), expected);
}
