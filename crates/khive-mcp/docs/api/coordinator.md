...
message and backend-id masking run over the full, untruncated text before any
length cap is applied — truncating first would let a detector's terminating
span (e.g. the `@` closing a `scheme://user:pass@host` credential) fall
outside the scanned window and leak.
