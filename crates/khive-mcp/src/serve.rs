...
/// must not start components; the skip is logged so the role decision is
/// visible. In daemon role the registry itself always logs the enumerated
/// roster (names + count), including an empty one.
fn start_daemon_components_if_daemon(args: &Args, server: &KhiveMcpServer) {
    if !args.daemon {
        tracing::info!(
            "daemon components: skipped (client role; daemon owns component supervision)"
        );
        return;
    }
