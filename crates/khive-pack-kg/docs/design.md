...
caller identity introspection (`whoami`), and WAL/checkpoint operator
diagnostics (`db_diagnostics`, ADR-091 — its PASSIVE probe may backfill WAL
frames but never changes logical state).
...
| [`src/handler_defs.rs`](../src/handler_defs.rs)                   | KG_HANDLERS static table (20 HandlerDef entries)         |
| [`src/handlers/mod.rs`](../src/handlers/mod.rs)                   | 20 verb handler implementations                          |
