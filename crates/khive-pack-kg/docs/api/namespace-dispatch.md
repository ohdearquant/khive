...
"lambda:leo"`, simulated via `--actor lambda:leo`) still routes both `create` and `list`
...
3. A direct-token `list` scoped to the actor namespace (e.g. `"lambda:leo"`) returns an empty
