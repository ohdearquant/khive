...
// the full-UUID contract. Distinct from feedback's visible-set policy
// (`brain_feedback_accepts_visible_only_target_id` in
// `khive-pack-brain/tests/dispatch_hook.rs`) — by-ID CRUD has no visibility
// boundary at all; the Gate is the authz seam.
