...
/// Validate an optional edge weight, defaulting to 1.0 if absent.
///
/// # Errors
///
/// Returns `RuntimeError::InvalidInput` if the weight is non-finite or outside `[0.0, 1.0]`.
fn validate_weight(weight: Option<f64>) -> Result<f64, RuntimeError> {
