...
pub(crate) static KG_HANDLERS: [HandlerDef; 24] = [
    ...
    HandlerDef {
        ...
        params: &[
            ...
            ParamDef { name: "observed", param_type: "array of object", required: false, description: "Atomic mode only: [{key, kind, version}], checked inside the transaction before the first write. A positive version asserts the live key's exact version; explicit null asserts no live holder. A mismatch refuses the batch with version_conflict naming the key and the entry index. The version field is required. Refused in per-member mode.", resolution_mode: IdResolutionMode::NotApplicable },
