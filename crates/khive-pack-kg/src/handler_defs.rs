...
pub(crate) static KG_HANDLERS: [HandlerDef; 24] = [
    ...
    HandlerDef {
        ...
        params: &[
            ...
            ParamDef { name: "observed", param_type: "array of object", required: false, description: "Atomic mode only: [{key, kind, version, id?}], checked inside the transaction before the first write. A positive version asserts the live key's exact version; explicit null asserts no live holder. A mismatch refuses the batch with version_conflict naming the key and the entry index. The version field is required. Optional id pins the note UUID and requires a positive version; a replacement refuses with identity_conflict before the version check. The refusal includes current_id only when the caller may learn the holder, under the same disclosure rule as key_conflict. Without id, the entry asserts version equality on the note holding the key at commit time. Refused in per-member mode.", resolution_mode: IdResolutionMode::NotApplicable },
