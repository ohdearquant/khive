...
### Reading a result from another pack backend

With configured pack runtimes, `get(id=...)` resolves live KG entities and
notes across their backends. A message found by search can therefore be read
even when `comm` and `kg` use different databases:

```text
request(ops='search(kind="message", query="handoff")')
# Replace <message-uuid> with a full UUID returned by search.
request(ops='get(id="<message-uuid>")')
request(ops='brain.feedback(target_id="<message-uuid>", signal="useful")')
```

The normal Gate still authorizes each operation; by-ID resolution does not
filter the stored namespace. An 8-or-more-character hex prefix must identify
one distinct UUID across the configured backends. Entity, note, edge and event
candidates, including sidecar events, participate in ambiguity detection;
backend failures propagate rather than appearing as a miss.

This shared read resolver fetches entity/note payloads. It does not reroute
`update` or `delete`, name/key lookup, or pack-private records. Existing
`include_deleted` ownership restrictions remain. Embedders install the resolver
through `PackRegistry::register_packs_with_runtimes`; ordinary single-runtime
registration retains its local behavior. See
