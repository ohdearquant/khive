...
| `db_diagnostics` | Writer-contention, graph-edge integrity, and WAL/checkpoint diagnostics: aggregate plus pooled/standalone/writer-task acquisition counters, pooled timeouts, swallowed audit failures, duplicate edge-ID and list-ledger counts, checkpoint counters, PASSIVE probe, WAL size, and explicitly qualified holder census (probe may backfill WAL frames; never TRUNCATE, never creates or deletes files) |
