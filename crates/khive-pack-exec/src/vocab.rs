...
pub static EXEC_HANDLERS: [HandlerDef; 9] = [
    ...
    HandlerDef {
        name: "exec.run",
        description: "Run one registered tool over a materialized tree inside the seatbelt sandbox under tool.check; returns {receipt, changed}. New receipts, including refusals, include effective_max_output_bytes, the resolved output retention cap in bytes for each stream for that run. Once the sandbox is prepared, sandbox.tool_source records the resolved registry exec:<absolute path> and sandbox.tool_registry_id its canonical row UUID, beside tool_binary_digest. These report the registration used, not a check against the caller's expected source; earlier failures keep sandbox null. Every refusal writes a receipt, and the refusal error carries its evidence as structured fields: code exec_refused, receipt_id=<id>, reason (the receipt's reason), effective_max_output_bytes (the receipt's cap), domain_disposition not_committed. The id also stays inside the message text as receipt_id=<id>, unchanged, for readers that already parse it; the fields are the contract and the wording is not. New receipts always include limiting_resource: cpu_seconds | address_space | file_size | nproc | null. Only a directly waited SIGXCPU with cpu_seconds configured or SIGXFSZ with file_size configured names a resource; normal exits, other signals and timeout record null. Child ENOMEM/EAGAIN are not visible to the wrapper, so address_space and nproc remain null (and are refused at startup on macOS). Older stored receipts are unchanged. The sandbox is macOS seatbelt only: a host without /usr/bin/sandbox-exec refuses the call at startup and names the platform.",
        visibility: Visibility::Verb,
    ...
    HandlerDef {
        name: "exec.receipt",
        description: "Read one run receipt by id. For a refused exec.run, take that id from the error envelope's receipt_id field rather than by matching the refusal sentence; the envelope's reason and effective_max_output_bytes equal this receipt's. New receipts, including refusals, include effective_max_output_bytes, the resolved output retention cap in bytes for each stream for that run. New receipts whose sandbox was prepared include sandbox.tool_source (the resolved registry exec:<absolute path>) and sandbox.tool_registry_id (its canonical row UUID) beside tool_binary_digest; earlier failures keep sandbox null. New receipts always include limiting_resource: cpu_seconds | address_space | file_size | nproc | null. Only a directly waited SIGXCPU with cpu_seconds configured or SIGXFSZ with file_size configured names a resource; normal exits, other signals and timeout record null. Child ENOMEM/EAGAIN are not visible to the wrapper, so address_space and nproc remain null (and are refused at startup on macOS). Older stored receipts are unchanged.",
        visibility: Visibility::Verb,
        category: VerbCategory::Assertive,
        params: &[
    ...
    HandlerDef {
        name: "exec.runs",
        description: "List run receipts for an actor, newest first, optionally filtered by tool and session. New receipts include effective_max_output_bytes, the stored output retention cap in bytes for each stream for that run; older receipts remain unchanged.",
        visibility: Visibility::Verb,
        category: VerbCategory::Assertive,
        params: &[
