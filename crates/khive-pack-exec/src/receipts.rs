...
/// Next per-session sequence number (1 for the first run of a session).
pub async fn next_seq(rt: &KhiveRuntime, ns: &str, session_id: &str) -> Result<i64, RuntimeError> {
    let mut reader = rt.sql().reader().await?;
    let rows = reader
        .query_all(SqlStatement {
            sql: "SELECT COALESCE(MAX(seq), 0) AS last FROM exec_runs \
            ...
            label: Some("exec_runs_next_seq".into()),
        })
        .await?;
    Ok(rows.first().and_then(|r| int(r, "last")).unwrap_or(0) + 1)
}

pub async fn insert(rt: &KhiveRuntime, ns: &str, receipt: &Value) -> Result<(), RuntimeError> {
...
pub async fn get(rt: &KhiveRuntime, ns: &str, id: &str) -> Result<Value, RuntimeError> {
    ...
    let rows = reader
        .query_all(SqlStatement {
            sql: "SELECT receipt FROM exec_runs WHERE namespace = ?1 AND id = ?2".into(),
            params: vec![
                SqlValue::Text(ns.to_string()),
...
pub async fn list(
    ...
    let mut sql = String::from("SELECT receipt FROM exec_runs WHERE namespace = ?1 AND actor = ?2");
    let mut params = vec![
        SqlValue::Text(ns.to_string()),
