...
graph writes. Target opening and schema initialization still occur. When L2 is disabled, its five
report counters are omitted so the existing default report shape is unchanged. Arguments are
deserialized through a closed typed schema (`path`, `db`,
