...
impl DeclKind {
    ...
    pub(crate) const fn code_token(self) -> &'static str {
        match self {
...
/// `source_ingest::resolve_l2_reference`, not any per-language Scanner.
...
pub(crate) struct ExtractedDeclaration {
    ...
    /// Call-target paths this declaration references (D3 rule 1: a function
    /// may depend on a callable function/datatype/interface). Resolution against the project's own
...
/// a different module than the impl block itself) happens in the ingest
/// pipeline, the same `crate`/`self`/`super`-aware resolver call resolution
/// uses.
#[derive(Debug, Clone)]
...
mod tests {
    ...
    #[test]
    fn code_token_round_trips_through_from_code_token() {
        for kind in [
            DeclKind::Function,
            DeclKind::Datatype,
            DeclKind::Interface,
            ...
            assert_eq!(DeclKind::from_code_token(kind.code_token()), Some(kind));
        }
        assert_eq!(DeclKind::from_code_token("bogus"), None);
    }
}
