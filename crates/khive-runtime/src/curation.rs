...
impl KhiveRuntime {
    ...
    pub async fn update_note_from_snapshot_with_embedding_report(
        ...
        let persisted = if self.stream_member_error(&note).await?.is_some() {
            self.sql().writer().await?.execute(
                khive_db::stores::note::note_metadata_replace_if_unchanged_statement(
                    &note, expected_updated_at, expected_deleted_at,
                ),
            ).await? == 1
        } else {
            store.replace_note_if_unchanged(note.clone(), expected_updated_at, expected_deleted_at).await?
        };
        if !persisted {
            return Err(stale_note_snapshot_error(id));
        }
