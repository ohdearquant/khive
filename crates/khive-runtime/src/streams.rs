...
use khive_storage::{AtomicUnitOp, Note, SqlRow, SqlStatement, SqlValue, StorageCapability, StorageError};
use khive_types::{Details, KhiveError};
use serde_json::{Value, json};

use crate::atomic_message::{AtomicNoteSpec, prepare_notes_atomic};
use crate::atomic_runner::AtomicOpPlan;
use crate::{KhiveRuntime, NamespaceToken, RuntimeError, RuntimeResult, micros_to_iso};

fn statement(sql: &str, params: Vec<SqlValue>) -> SqlStatement {
    SqlStatement { sql: sql.into(), params, label: Some("stream".into()) }
}

fn validate_stream(stream: &str) -> RuntimeResult<()> {
    if stream.len() > 512 || stream.contains('\0') {
        return Err(RuntimeError::InvalidInput("stream must be at most 512 UTF-8 bytes and contain no U+0000".into()));
    }
    Ok(())
}
...
fn integer(row: &SqlRow, name: &str) -> RuntimeResult<i64> {
    match row.get(name) {
        ...
        _ => Err(RuntimeError::Internal(format!("stream query missing integer {name}"))),
    }
}
...
fn text<'a>(row: &'a SqlRow, name: &str) -> RuntimeResult<&'a str> {
    match row.get(name) {
        ...
        _ => Err(RuntimeError::Internal(format!("stream query missing text {name}"))),
    }
}

fn write_failure(message: &str) -> StorageError {
    StorageError::Conflict { capability: StorageCapability::Notes, operation: "stream.append".into(), message: message.into() }
}
...
impl KhiveRuntime {
    ...
    pub async fn stream_append(
        ...
        let content = serde_json::to_string(record).map_err(|e| RuntimeError::InvalidInput(e.to_string()))?;
        let mut prepared = prepare_notes_atomic(self, vec![AtomicNoteSpec {
            token, id: None, kind: note_kind, name: None, content: &content,
        ...
        let AtomicOpPlan::AddNote(plan) = prepared.plans.remove(0) else {
            return Err(RuntimeError::Internal("stream preparation did not produce a note".into()));
        };
        ...
        let op: AtomicUnitOp = Box::new(move |writer| Box::pin(async move {
            let scope = vec![SqlValue::Text(ns.clone()), SqlValue::Text(stream_owned.clone())];
            let head = writer.query_scalar(statement(
                "SELECT COALESCE(MAX(seq), 0) FROM note_streams WHERE namespace=?1 AND stream=?2", scope,
            )).await?;
            let Some(SqlValue::Integer(head)) = head else { return Err(write_failure("invalid stream head")); };
            let next = head.checked_add(1).ok_or_else(|| write_failure("stream sequence exhausted"))?;
            ...
            for planned in plan.statements {
                ...
                if planned.guard.is_some_and(|guard| !guard.holds_for(affected)) {
        ...
        let outcome = self.sql().atomic_unit(op).await?.downcast::<AppendOutcome>()
            .map_err(|_| RuntimeError::Internal("invalid stream append outcome".into()))?;
        match *outcome {
            AppendOutcome::Appended(seq) => Ok(json!({"seq": seq, "id": note.id, "created_at": micros_to_iso(note.created_at)})),
            AppendOutcome::Conflict(next) => Err(KhiveError::conflict("stream sequence precondition failed").with_details(Details::new_owned([
                ("reason", "seq_conflict".into()), ("stream", stream.into()),
                ("expected_seq", expected_seq.expect("only conditional appends conflict").to_string()), ("next_seq", next.to_string()),
            ])).into()),
        }
    }
    ...
    pub async fn stream_read(&self, token: &NamespaceToken, stream: &str, after: i64, limit: i64) -> RuntimeResult<Value> {
        validate_stream(stream)?;
        if after < 0 || limit < 1 {
            return Err(RuntimeError::InvalidInput("stream.read requires after >= 0 and limit >= 1".into()));
        }
        ...
        let head = rows.first().map(|row| integer(row, "head_seq")).transpose()?.unwrap_or(0);
        let mut entries = Vec::new();
        let mut last = None;
        for row in &rows {
            if matches!(row.get("seq"), Some(SqlValue::Null)) { continue; }
            let seq = integer(row, "seq")?;
            let record: Value = serde_json::from_str(text(row, "content")?).map_err(|e| RuntimeError::Internal(format!("invalid stream record JSON: {e}")))?;
            entries.push(json!({"seq": seq, "id": text(row, "id")?, "record": record, "created_at": micros_to_iso(integer(row, "created_at")?)}));
            last = Some(seq);
        }
        Ok(json!({"entries": entries, "head_seq": head, "next_after": last.filter(|last| *last < head)}))
    }
    ...
    pub(crate) async fn stream_member_error(&self, note: &Note) -> RuntimeResult<Option<RuntimeError>> {
        let row = self.sql().reader().await?.query_row(statement(
            "SELECT stream,seq FROM note_streams WHERE namespace=?1 AND note_id=?2",
            vec![SqlValue::Text(note.namespace.clone()), SqlValue::Text(note.id.to_string())],
        )).await?;
        row.map(|row| Ok(KhiveError::conflict("stream entries are immutable").with_details(Details::new_owned([
            ("reason", "stream_member".into()), ("id", note.id.to_string()),
            ("stream", text(&row, "stream")?.into()), ("seq", integer(&row, "seq")?.to_string()),
        ])).into())).transpose()
    }
}
...
mod tests {
    ...
    use crate::atomic_runner::{AtomicRunOutcome, run_atomic_unit};
    ...
    async fn stream_atomic_metadata_cas_preserves_record_and_refuses_stale_plan() {
        ...
        let appended = rt.stream_append(&token, "cas", &json!({"n": 1}), None, "observation", None).await.unwrap();
        let id = uuid::Uuid::parse_str(appended["id"].as_str().unwrap()).unwrap();
        for args in [json!({"id": id, "content": "changed"}), json!({"id": id, "properties": {"x": 1}})] {
        ...
        for hard in [false, true] {
            let err = prepare_delete(&rt, &token, &json!({"id": id, "hard": hard}), None).await.unwrap_err();
            assert!(err.to_string().contains("immutable"));
        }
        let plan = prepare_update(&rt, &token, &json!({"id": id, "salience": 0.7}), None).await.unwrap();
        assert!(matches!(run_atomic_unit(rt.sql().as_ref(), vec![plan]).await.unwrap(), AtomicRunOutcome::Committed { .. }));
        let stale = prepare_update(&rt, &token, &json!({"id": id, "salience": 0.2}), None).await.unwrap();
        rt.update_note(&token, id, NotePatch::new(None, None, Some(Some(0.9)), None, None)).await.unwrap();
        assert!(matches!(run_atomic_unit(rt.sql().as_ref(), vec![stale]).await.unwrap(), AtomicRunOutcome::RolledBack { .. }));
        let note = rt.notes(&token).unwrap().get_note(id).await.unwrap().unwrap();
        assert_eq!(note.salience, Some(0.9));
        assert_eq!(serde_json::from_str::<Value>(&note.content).unwrap(), json!({"n": 1}));
    }
    ...
    async fn stream_namespace_isolation_and_real_stat_count() {
        ...
        for token in [&a, &b] {
            assert_eq!(rt.stream_append(token, "same-name", &json!(token.namespace().as_str()), Some(1), "observation", None).await.unwrap()["seq"], 1);
        }
        for token in [&a, &b] {
        ...
        assert_eq!(rt.stream_stat(&a, "same-name").await.unwrap(), json!({"count": 1, "head_seq": 5}));
        assert_eq!(rt.stream_stat(&b, "same-name").await.unwrap(), json!({"count": 1, "head_seq": 1}));
    }
}
