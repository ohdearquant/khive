...
`try_normalize_weights` rejects the first non-finite value by index. `normalize_weights` is the
lossy normalization described above, and `weights_are_normalized` checks whether the positive sum
