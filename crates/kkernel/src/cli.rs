...
//!
//! The `kkernel` binary (`src/main.rs`) is a thin shim over [`run`]. See
//! `crates/kkernel/docs/usage.md` for the full subcommand reference.
...
/// Run the `kkernel` CLI: parse args, resolve the effective subcommand, and
/// dispatch it. The binary's `main` (`src/main.rs`) does nothing but
/// construct the tokio runtime and call this.
