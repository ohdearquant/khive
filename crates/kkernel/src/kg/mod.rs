...
/// Map a validated severity string to a `&'static str`.
///
/// Callers must validate with [`validate_severity`] first; this function
/// treats any unrecognized value as `"warning"` for backward compatibility
/// in internal paths that bypass validation.
...
/// Validate that `s` is one of the three accepted severity levels.
///
/// Returns `None` for valid values, or an error `RuleResult` for invalid ones.
...
fn configurable_rule_checks(
    ...
    for rule in &rules_file.rules {
        // Validate severity before evaluating the rule.
