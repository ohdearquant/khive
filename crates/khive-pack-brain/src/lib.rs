//! Brain pack — profile-oriented Bayesian auto-tuning for khive.
//!
...
pub(crate) use crate::handlers::BRAIN_HANDLERS;

pub(crate) const ENTITY_CACHE_CAPACITY: usize = 10_000;
...
///
/// Called from both the feedback path and the dispatch hook path so the record
/// is never stale regardless of which path updated the state. Also called from
/// the reset path so the profile record reflects restored priors immediately.
...
mod tests {
    use super::*;
    use khive_runtime::{DispatchHook, Namespace, PackRuntime, VerbRegistryBuilder};
    use serde_json::json;
