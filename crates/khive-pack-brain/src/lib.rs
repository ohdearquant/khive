...
impl BrainPack {
    ...
    async fn handle_feedback(&self, params: Value) -> Result<Value, RuntimeError> {
        #[derive(Deserialize)]
    ...
    async fn handle_emit(&self, params: Value) -> Result<Value, RuntimeError> {
        self.handle_feedback(params).await
    }
...
impl PackRuntime for BrainPack {
    ...
    async fn dispatch(
        ...
        match verb {
            ...
            "brain.feedback" => self.handle_feedback(params).await,
            // Declaration
...
mod tests {
    ...
    async fn dispatch_profiles_returns_default_profile() {
        ...
        let registry = empty_registry();
        let result = pack
            .dispatch("brain.profiles", json!({}), &registry)
            .await
            .unwrap();
    ...
    async fn dispatch_profiles_filtered_by_lifecycle() {
        ...
        let registry = empty_registry();
        let result = pack
            .dispatch("brain.profiles", json!({"lifecycle": "active"}), &registry)
            .await
            .unwrap();
        let profiles = result["profiles"].as_array().unwrap();
        for p in profiles {
    ...
    async fn dispatch_profile_not_found_returns_not_found() {
        ...
        let registry = empty_registry();
        let err = pack
            .dispatch("brain.profile", json!({"id": "nonexistent"}), &registry)
            .await
            .unwrap_err();
    ...
    async fn dispatch_unbind_removes_binding() {
        ...
        // Remove it
        let result = pack
            .dispatch("brain.unbind", json!({"actor": "agent-y"}), &registry)
            .await
            .unwrap();
    ...
    async fn dispatch_config_all_parameters() {
        ...
        let registry = empty_registry();
        let result = pack
            .dispatch("brain.config", json!({}), &registry)
            .await
            .unwrap();
