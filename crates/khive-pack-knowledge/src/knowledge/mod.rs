...
use crate::knowledge::schema::{
    ...
    ImportParams, IndexParams, ListParams, ReindexParams, SearchParams, Section, SectionType,
...
impl KnowledgeHandlers {
    ...
    pub(crate) async fn reindex(
        ...
        let p: ReindexParams = deser(params)?;

        // Resolve model name: explicit param wins, then runtime default.
        let model_name: String = match p.model.as_deref().filter(|s| !s.is_empty()) {
            Some(name) => name.to_string(),
            None => {
                let default = runtime.default_embedder_name();
                if default.is_empty() {
                    return Ok(json!({
        ...
        let batch_size = p.batch_size.unwrap_or(100).clamp(1, 500) as u32;
        ...
        loop {
            ...
            // Build (id, embed_text) pairs — skip empty texts.
            ...
            if !staged.is_empty() {
                ...
                match runtime.embed_batch_with_model(&model_name, &texts).await {
                    Ok(embeddings) if embeddings.len() == staged.len() => {
                        match runtime.vectors_for_model(token, &model_name) {
                                ...
                                for ((id, _), emb) in staged.iter().zip(embeddings.iter()) {
                                        ...
                                        .await
                                    {
                                        tracing::warn!(
                                            entity_id = %id,
                                            error = %e,
                                            "reindex: entity vector insert failed, skipping"
                                        );
                                        errors_skipped += 1;
                                    }
                                }
                            ...
                            Err(e) => {
                                tracing::warn!(
                                    error = %e,
                                    "reindex: failed to get vector store for model, skipping batch"
                                );
                    ...
                    Ok(_) => {
                        tracing::warn!(
                            "reindex: embedding count mismatch for entity batch, skipping"
                        );
                        errors_skipped += staged.len() as u64;
                    }
                    Err(e) => {
                        tracing::warn!(
                            error = %e,
                            "reindex: entity embed_batch failed, skipping batch"
                        );
                        errors_skipped += staged.len() as u64;
                    }
                }
        ...
        loop {
            ...
            if !staged.is_empty() {
                ...
                match runtime.embed_batch_with_model(&model_name, &texts).await {
                    Ok(embeddings) if embeddings.len() == staged.len() => {
                        match runtime.vectors_for_model(token, &model_name) {
                                ...
                                for ((id, _), emb) in staged.iter().zip(embeddings.iter()) {
                                        ...
                                        .await
                                    {
                                        tracing::warn!(
                                            note_id = %id,
                                            error = %e,
                                            "reindex: note vector insert failed, skipping"
                                        );
                                        errors_skipped += 1;
                                    }
                                }
                            ...
                            Err(e) => {
                                tracing::warn!(
                                    error = %e,
                                    "reindex: failed to get vector store for model, skipping note batch"
                                );
                                errors_skipped += staged.len() as u64;
                            }
                        }
                    ...
                    Ok(_) => {
                        tracing::warn!(
                            "reindex: embedding count mismatch for note batch, skipping"
                        );
                        errors_skipped += staged.len() as u64;
                    }
                    Err(e) => {
                        tracing::warn!(
                            error = %e,
                            "reindex: note embed_batch failed, skipping batch"
                        );
                        errors_skipped += staged.len() as u64;
                    }
                }
