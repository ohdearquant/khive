...
`begin(phase)` must be called _before_ that phase's work starts (in particular, before any
...
`begin(phase)` returns `Result` and refuses a new phase after the merged
request-read cancellation signal or absolute deadline fires. The compose
handler repeats that check after awaited suggest/embed/KG operations, around
domain and atom fetch loops, and inside synchronous tokenization, BM25, and
section scoring. Ordinary suggest or KG-blend failures may still degrade to a
useful response; cancellation/deadline is not degradable and must propagate
before another read phase starts.
...
Records the query's UTF-8 _byte_ length, not a char count — `str::len()` reads a value the
...
the real delivery mechanism there. The return value exists so tests can assert the _actual_
