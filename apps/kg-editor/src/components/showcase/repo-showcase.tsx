  ...
  return (
    <div className="repo-view-body repo-grid two">
      ...
      <section className="repo-card">
        ...
        <div className="repo-list">{cycleRows.map((cycle) => <div className="repo-list-row" key={cycle.id}><GitFork aria-hidden="true" /><div><span className="repo-cycle-members repo-cycle-label">SCC members: {cycle.module_ids.map((id, index) => <span key={id}><ModuleInspectionControl moduleId={id} moduleById={moduleById} modulePage={bundle.graph.modules} selectedModuleId={selectedModuleId} onInspectModule={onInspectModule} className="compact" />{index < cycle.module_ids.length - 1 ? " · " : ""}</span>)}</span><code className="repo-analysis-id" title={cycle.id}>{cycle.id}</code></div></div>)}</div>
...
function TreemapView({ bundle, moduleById, selectedModuleId, onInspectModule }: ViewProps) {
  ...
  const packageById = new Map(
    bundle.graph.packages.items.map((packageNode) => [
      packageNode.id,
      packageNode,
    ]),
  );
  const layout = buildStructureTreemap(rows.map((row) => {
    const moduleNode = moduleById.get(row.module_id);
    const packageLabel = packageById.get(row.package_id)?.name ?? row.package_id;
    return {
