...
| `content`           | string          | no          | Required body text for ordinary notes; loaded task/finding hooks may derive it.                                                                                                                                                                                                            |
| `salience`          | number          | no          | Ordinary-note salience, 0.0–1.0. Task salience is derived from `priority`, so task creates reject an explicit value.                                                                                                                                                                        |
...
| `description`       | string          | no          | Entity free-text description, or task body text when the GTD task hook is loaded. Other note kinds reject it instead of silently dropping it.                                                                                                                                              |
| `title`             | string          | no          | Task/finding hook title; usable instead of `name` for those note kinds.                                                                                                                                                                                                                      |
| `priority`          | string          | no          | Task-hook priority (`p0`–`p3`).                                                                                                                                                                                                                                                             |
| `status`            | string          | no          | Task-hook initial lifecycle status.                                                                                                                                                                                                                                                         |
| `assignee`          | string          | no          | Task-hook assignee.                                                                                                                                                                                                                                                                         |
| `due`               | string          | no          | Task-hook due timestamp.                                                                                                                                                                                                                                                                    |
| `start` / `end`     | string          | no          | Task-hook scheduling metadata.                                                                                                                                                                                                                                                              |
| `depends_on`        | array\<string\> | no          | Task-hook dependency references, validated before create.                                                                                                                                                                                                                                  |
| `context_entity_id` | string          | no          | Task-hook context entity reference; creates an `annotates` edge after commit.                                                                                                                                                                                                               |
...
| `annotates`         | array\<string\> | no          | Singleton note annotation target references.                                                                                                                                                                                                                                               |
...
When `items` is present, record payload fields belong inside each item; misplaced
singleton fields such as `annotates`, `edges`, or `skip_dedup_check` are rejected.
Conversely, `atomic` and `verbose` are rejected when `items` is absent.
...
For `atomic=true`, note FTS rows are part of the atomic commit; vector stages and
hook-owned `annotates`/`after_create` side effects may be reported after commit.
Post-commit vector work follows metadata-only note revisions under a full
revision witness because those updates do not reindex themselves. A text/name
update owns its replacement indexes, and a deleted note is skipped, so stale
work cannot overwrite newer text vectors or resurrect a tombstoned row.
