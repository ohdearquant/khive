...
| `db`        | string          | no       | Existing target map database with the complete canonical ledger for the current khive schema. A missing, empty, stale, newer, or noncanonical explicit target is rejected without creation or migration; the raw writer revalidates before write-intent PRAGMAs. Initialize/upgrade an older target with `kkernel db migrate --db <path>`, or use a khive build supporting a newer target. Omit `db` to create or migrate `<path>/.khive/code-map.db`. The shared production database — its default `$HOME/.khive/khive.db` location and the calling server's actual configured database — is always rejected, with no override. |
