...
### D5: Cross-ontology alignment lives in the Subject's OntologySpec

The mapping from a Subject's native subtypes and relations to external ontology IRIs is declared
in that Subject's OntologySpec (ADR-072), alongside the internal validation rules. The
OntologySpec already declares what a vertical's vocabulary is; the alignment table extends it
with the external IRI each token corresponds to and the mapping predicate (`sameAs`,
`exactMatch`, `broader`, `narrower`). One artifact, double duty: internal validation and external
alignment.
...
### Why alignment belongs in the OntologySpec

The OntologySpec is already the per-vertical vocabulary source of truth (ADR-072). External
alignment is the same vocabulary, mapped outward. Keeping it in one artifact avoids a second
registry that must be kept in sync with the first.
...
**A4: A standalone alignment registry separate from the OntologySpec.** Rejected. This splits the
vocabulary contract across two artifacts that must agree. The OntologySpec is already the
per-vertical source of truth (ADR-072); alignment belongs there.
...
2. Generate the khive OWL/RDFS/SKOS vocabulary from the closed enums and pack/Subject
   declarations.
3. Build the RDF exporter (Turtle and JSON-LD) over a namespace's entities and edges.
4. Extend the OntologySpec schema (ADR-072) with an optional alignment table: khive token ->
...
- ADR-072: Subject OntologySpec as Runtime Data -- the artifact the alignment table extends
