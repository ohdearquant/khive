...
2. **Lifecycle** — what state transitions are legal (inbox → next → active → done for
   GTD's `task` kind; permanent for `decision` kind; decay-eligible for `memory`).
...
2. **Pack extensibility.** GTD adds `task`. Memory adds `memory`. Brain adds its own.
   Future packs add theirs. The taxonomy cannot be a closed Rust enum.
...
Other packs register their own kinds, additive to the base five:

| Kind     | Owner               | Source                 | Purpose                                                    |
| -------- | ------------------- | ---------------------- | ---------------------------------------------------------- |
| `task`   | `khive-pack-gtd`    | **ADR-019** (GTD Pack) | GTD-style work items with inbox/next/active/done lifecycle |
| `memory` | `khive-pack-memory` | ADR-021                | Decay-weighted memory notes with `remember`/`recall` verbs |

These are not "second-class kinds." They are full peers of the base five — same
...
KG-only deployments (default) see only the base five. `KHIVE_PACKS=kg,gtd,memory`
adds `task` and `memory` to the active set.
...
**Annotations do not transfer through supersession.** Annotations are per-note and
...
packs may define their own defaults when called through their own verbs (e.g., `remember`
defaults to `memory`, `assign` defaults to `task`).
...
the caller wants a research-focused subset without including procedural records like
`task` or `memory`.

### Aliases

`NoteKindSpec` does not standardize aliases. Each pack documents the canonical kind
string only — `observation`, `insight`, `question`, `decision`, `reference` for the
base five; `task`, `memory` for the others. The MCP wire layer accepts only the
canonical form.

This rejects the earlier alias surface (`obs`, `q`, `ref`) because:

- Aliases multiply across packs (gtd's `task` would need `todo`, `t`, `work` —
  every pack invents its own).
...
- Pack-extensible: GTD, memory, brain, and future packs add kinds without changing
  foundational crates.
...
- ADR-019: GTD Pack — `task` kind registration.
- ADR-021: Memory Pack Semantics — `memory` kind registration.
