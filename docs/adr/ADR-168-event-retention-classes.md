...
receipt (`crates/khive-runtime/src/pack.rs`'s `persist_git_digest_receipt`) and, on any path
where that receipt cannot be built, by the same generic dispatch/error audit
(`persist_intercepted_audit`). A `(kind, verb)` pair with more than one production emitter is
...
| 5  | `Audit`                         | `git.digest` — **collapsed row, two production emitters for the same `(kind, verb)` key**: `verb="git.digest"`, produced by both (a) the pinned digest receipt (`crates/khive-runtime/src/pack.rs`'s `persist_git_digest_receipt`, ADR-088 Amendment 1) on the success path, and (b) the generic dispatch/error audit (`persist_intercepted_audit`, the same site as row 1) whenever the receipt cannot be built (`GitDigestReceiptOutcome::PersistenceUnavailable` or `BuildRejected`) or an error occurs before the receipt path runs (`crates/khive-runtime/src/pack.rs:1497-1509`). Write posture: coupled-outcome (ADR-162 §4 third bullet) for the receipt emitter. | `pinned_while_referenced` (provisional)   | none            | Referent: the ingested project/digest record (`project_id`), for rows written by the receipt emitter; rows written by the fallback generic-audit emitter carry no referent, but the collapsed row is still classified at the safest of the two — `pinned_while_referenced` dominates `age_archivable`. Terminal: not yet named; a superseding receipt for the same `project_id` may or may not retire the prior one, and confirmation belongs to a git-pack amendment of this row. |
...
| 52 | `ArchiveSegmentAccessAttempted` (this ADR, §5, §6; not yet in `EventKind::ALL`) | Operator unseal/restore/replace/delete action, one per attempt. | `pinned_while_referenced` | none            | Same shape as row 50.                                                                                                                         |
...
observation-decoding branch in `crates/khive-db/src/stores/event.rs`. Until all four land
...
- **`ArchiveSegmentAccessAttempted`**: one per unseal, restore, replace, or delete attempt
  ...
  - **`true`** or **`false`** is the value for an allowed `unseal` or `restore`, reflecting
    whether the recomputed content digest matched the segment's manifest digest (§3's
    verify step, applied here to a read rather than a prune).
  - **`unknown`** is also the value for an allowed `replace_segment` or `delete_segment`,
    ...
    comparison to report for these two actions, allowed or denied.
...
- **Replace (`archive.replace_segment`).** A new segment supersedes a sealed one; this is a
  **transition**, not a raw overwrite. It specifies:
  ...
  - **Manifest versioning.** The original sealed segment's manifest is preserved as
    immutable history — it is never deleted or rewritten by a replace — and a new manifest
    version is written that points from the original `segment_id` to the replacement
    `segment_id`. A reader resolving the original id after a replace follows this pointer;
    the original manifest itself continues to state what it always stated.
  - **Post-write verification.** After the version pointer is written, the replacement
    segment is independently re-verified per §3 (recomputed digest and row count against
    its own manifest) before the replace is considered complete.
  - **Failure recovery.** If any step above fails — replacement sealing, verification, or
    the version-pointer write — the **original segment remains authoritative**: nothing in
    this action removes or disables the original ahead of a successfully verified
    replacement, so a failed replace leaves the pre-replace state fully intact and
    resolvable.
...
- **Deny path (any action):** Gate decision (`deny`) → `ArchiveSegmentAccessAttempted`
  ...
  runs.
- **Allow path, open/restore:** Gate decision (`allow`) → content digest recomputed against
  the segment → `ArchiveSegmentAccessAttempted` appended with `gate_decision: "allow"` and
  `digest_verified` set to the recomputation's actual result (`"true"` or `"false"`) →
...
- **Allow path, replace/delete:** Gate decision (`allow`) → `ArchiveSegmentAccessAttempted`
  ...
  replace transition above, or the terminal delete) runs.
...
unverified read for a verified one. A successful open or restore records outcome
`EventOutcome::Success` with `digest_verified: "true"`.
