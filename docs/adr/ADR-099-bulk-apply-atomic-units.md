# ADR-099: Cross-op atomicity for bulk apply with prepared write plans
...
This is acceptable for idempotent mass updates (one process instead of N spawns, parse-first
safety, and dry-run preview). It is **not** acceptable for schema normalization or relation repair
that requires all-or-nothing semantics. Those operations mutate existing rows through `update`,
`delete`, and `link`; they are not create-then-reference chains with intra-file dependencies.
...
constraint that governs this entire ADR**: on the single-writer daemon path, the
...
rusqlite calls that never await). A closure that performs asynchronous work such as embedding,
ANN warming, or a network or channel round-trip suspends, hits the error path, and fails.
...
> something the type system enforces. Every decision in this ADR that touches the seam (D1, D3)
...
Any design that wraps those handlers in one
...
Add cross-op atomicity as an **opt-in, prepared-write-plan** unit.

- **D1 (mechanism)**: `--atomic` runs in two phases. First, an **async prepare pass** materializes
  each admissible op into a synchronous write plan. All suspension-bearing work, including
  validation, identifier generation, and embedding where needed, happens outside any
...
contract in ADR-016/ADR-023 and the public tool description are untouched.
...
1. **Prepare pass (async, per op, outside any transaction).** Each admissible op runs its verb's
   `prepare` step, using the same validation, identifier generation, secret masking, and, where
   required, embedding computation as the handler. It produces a materialized **write plan**:
   the concrete set of `SqlStatement`s that op will apply (base rows, FTS rows,
...
- **Prepare/commit staleness.** In two-phase, an op's write plan is materialized in the prepare
  ...
  **Content-derived staleness** (a precomputed value goes stale): if a _later_ op in the same file
  ...
  later op overwrote. For example, an embedding computed for content C could be committed while a
  later op sets the content to C'. **v1 is immune to this class:** the v1 admissible set is DML-only (D3), so
...
- **Whole-file, not per-chunk.** The 100-op chunk is a transport-framing constant; all-or-nothing
  across a chunk boundary is not a semantically meaningful unit. A relation-repair file is atomic as a
  whole or not at all.
...
- **v1 admissible (explicit per-verb list, never a pack-level category):** `update`, `delete`,
  `link`, `merge`, and the governance verbs (`propose`/`review`/`withdraw`): the verbs whose
  prepare is validation and identifier resolution only and whose apply is pure DML.
- **`update` and `merge` caveat (verified at source): applies to BOTH substrates.** Under the
  ...
  **note** `update` triggers a reindex when note name or content change (`reindex_entity` /
  ...
  same split: their write plans carry the row/FTS DML; any needed entity or note reindex is
...
- **v1 rejected: embedding-bearing:** `create` (entity/note/document) computes embeddings and
  warms ANN as part of the write. It is excluded from `--atomic` v1
  because its prepare seam (hoisting embedding out of the transaction) is not yet built. An
...
- **v1 rejected: reads:** `search` / `query` / `traverse` / `list` / `get` /
...
The admissibility of a verb is a static property declared per verb as pack metadata: its apply
phase must honor the _atomic-unit suspend-free invariant_ by reducing to synchronous DML with no
in-transaction await. The parser consults that metadata and rejects any inadmissible op before
...
This is deliberately deferred because it is a larger per-handler refactor, while the immediate
schema-normalization and relation-repair cases are satisfied by the DML-only set.
...
- Expected daemon-side behavior during the window: daemon writes wait on the configured
  ...
  directs operators to run large atomic maintenance jobs in a maintenance window or against an idle daemon.
...
0. **Document the seam invariant in code.** Add a doc-comment to `SqlAccess::atomic_unit` (the trait
   ...
   real `await` (embedding, ANN, service, or channel) fails on the single-writer path. The
   regression test must enforce the same invariant on every execution mode.
1. Add a per-verb `prepare`/apply seam to the v1 admissible verbs (`update`, `delete`, `link`,
   `merge`, and the governance verbs): `prepare` runs the
...
5. Grep for remaining non-test `begin_tx`/`SqlTransaction` callers; remove `begin_tx` from
   `SqlAccess` and its implementation if none remains, else document the callers.
...
- **Suspend-trap regression test (fails loudly, never silently).** Two paired assertions enforce
  ...
  embedding is caught by a test, not after deployment on the single-writer path.
- **Embedding-bearing verb rejected.** An `--atomic` file containing `create` is rejected before
  execution, naming the line and verb and stating embedding-bearing verbs are not yet
  atomic-eligible.
- **Read verb rejected.** An `--atomic` file containing a read/query verb is rejected before
...
- **Revert-companion test.** The atomic-rollback test demonstrably **fails** against the pre-change
  per-op non-atomic bulk-apply shape, proving the test is non-vacuous.
...
3. **`SqlTransaction` retention.** Whether the `SqlTransaction` trait is forward-deployed
   ...
   collapse. Resolved by the implementation-time grep in migration step 5.
...
## Amendment 1 (2026-07-07): single-sourced prepare and apply logic

The atomic and singleton paths share each admissible verb's validation, identifier resolution,
idempotence checks, and statement generation. This is a binding single-source invariant:

1. `update` and `delete` use shared decision functions over the same store-owned
   `SqlStatement` builders for entity, note, and edge records.
2. `link` uses one shared metadata-merge function, and every event append uses the shared
   `event_insert_statements` builder.
3. Prepare functions are asynchronous and read-only. The atomic unit applies their plans in a
   suspend-free synchronous closure. Singleton handlers call the same prepare and apply logic
   inside their own transaction.
4. Symmetric-edge natural-key conflicts are resolved at commit time with guarded statements.
   Affected-row guards prove which conflict arm ran, and result rendering resolves the surviving
   edge identifier after commit instead of trusting a prepare-time advisory value.
5. The singleton upsert, atomic guarded insert, and batch upsert consume one shared builder for
   edge natural-key conflict SQL.

Tests for singleton and atomic execution use the same behavior matrix. A change to shared prepare
logic is accepted only when both paths continue to produce equivalent validation, mutation, event,
