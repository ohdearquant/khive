...
KG state lives as sorted NDJSON files in a git repository. Git provides commits, branches,
merges, remotes, and access control, while standard repository hosting can provide review
and CI. What ADR-010 does not specify is the concrete file format, the
...
MCP clients connect directly to `kkernel mcp`. Interactive CLI callers use `khive`. See
...
independent KGs live in separate repositories, with cross-repo edges (§8) handling the
...
1. New entity additions land at deterministic non-overlapping positions in the file. Two
   contributors on different branches adding different entities produce a clean three-way git
...
[ADR-023](./ADR-023-declarative-pack-format.md)). `entity_kinds` is the closed set from
ADR-001 plus the `resource` kind added by ADR-048, for a current total of nine. Only
`entity_types` is pack-extensible.
...
| `khive kg sync [--quiet] [--force]`                                                   | Atomic rebuild of `working.db` from NDJSON                                                          |
...
1. Run §7 status diff. If non-empty: refuse with "Uncommitted changes in working.db. Run
   `khive kg commit` or explicitly run `khive kg sync --force`."
...
`khive kg sync --force` runs the checkout flow against the committed NDJSON, discarding
uncommitted DB changes. The explicit flag distinguishes this destructive operation from a
routine sync.
...
```
~ entity 671b882a (concept "Example Technique")
    properties.status: "researched" → "implemented"

+ entity a3f2c1d4 (concept "Example Variant")
    name: Example Variant
    kind: concept
    entity_type: technique

- edge 671b882a --[competes_with]--> c9e4b3f2 (Example Variant)
```
...
```json
{
  "edge_id": "...",
  "source": "671b882a-...",
  "target": "kg://reference-kg/default/c9e4b3f2-...",
  "relation": "implements",
  "weight": 1.0,
  "properties": {}
}
...
`reference-kg` resolves through `schema.yaml#remotes`:
...
- `--merge-properties`: merge non-overlapping property changes from both sides; for
  ...
  merges where both branches extended the same entity's properties in
...
changes the files. They work with every git interface, including CLI, IDE git clients, and
GUI tools, without requiring the user to remember a wrapper command.
...
operations should not pass through the MCP surface. `khive kg import` reads
...
These five topics, file format, storage model, CLI workflow, branching, and schema evolution,
...
Every git tool, including CLI, IDE clients, GitHub Desktop, GitHub PR review, and CI runners,
already understands git branches. A custom KG branch concept that lives in a SQLite table would
...
| Pull = fetch + auto-merge (git's default)                         | Hides merge decisions; wrong for research KG correctness. Pull-as-fetch + explicit merge keeps the caller in control.                                          |
...
The Rust `kkernel` CLI ships through the per-platform npm subpackages defined by ADR-026:
...
The npm umbrella package selects the matching precompiled Rust binary for the host platform.
The binary target and optional-dependency contract are owned by ADR-026.
