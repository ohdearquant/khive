...
Embedding vectors are only comparable if produced by the same model. If developer A commits with
`mE5-small` (384 dimensions) and developer B syncs with `BGE-small` (same dimension count but a
...
[embed]
model = "mE5-small"        # provider model name; must match [[engines]] entry
...
For every config key: **CLI flag > project `khive.toml` > global `khive.toml` > `KHIVE_*`
environment variable > built-in default**. A missing key at any level falls through to the
next. When both file levels specify the same key, the **project level wins**. This ensures
project maintainers can lock the embedding model for consistency while users retain the
ability to set their inference device locally without touching committed files.
...
`embed.fields.include` with a single space separator. Calls the configured embedding
provider (ADR-011) in
...
For an entity with `name = "Example Technique"` and `description = "A documented
retrieval technique"`, the concatenated input is:
`"Example Technique A documented retrieval technique"`.
...
`auto_embed = false` and call `kkernel kg embed` on their own cadence.
...
- `embed.model` is a non-empty string. Model availability is validated by the configured provider
...
`[[engines]]` (ADR-028 and ADR-031) declares the process-wide registry of loaded embedding
models: logical names, provider model identifiers, dimensions, and optional fusion weights.

`[embed]` (this ADR) specifies which model is used for the entity-text embedding pipeline
and what fields it operates on. `embed.model` must reference a `model` value in
`[[engines]]`. The
...
- `~/.khive/khive.toml` introduces user-level config that must be documented and
  supported. A misconfigured `embed.device` produces a runtime error from the embedding provider
  rather than a config validation error. Mitigation: type validation catches `device`
  value errors at startup; model availability errors from the provider are propagated
...
1. **`[embed.fields.include]` as a pack-level field.** For extensions with non-standard
   entity schemas, such as records with `title` and `body` instead of `name` and
...
3. **`embed.dimensions` validation against the actual model.** At startup, the CLI could
   query the embedding provider for the model's output dimension and compare it to
...
- [ADR-011](./ADR-011-embedding-and-inference.md): embedding-provider boundary;
  `embed_missing` requests batched inference
