# ADR-113: Identifier continuity for merged-entity redirects and split endpoint moves
...
So any holder of `C`, whether a UUID copied into a free-text note body, an external reference, or a
cached id from before the merge, loses the entity even though the substrate knows exactly where it
went. This is the merge-dangling class.

### Split re-mints edge identifiers and orphans edge annotations

A split keeps entity `C`'s UUID, mints a new entity `D` of a different kind, and moves a subset of
`C`'s edges to `D`. It has no id-preserving edge-move on the public surface. `update` patches an
...
**Redirect response.** The caller receives an **explicit redirect marker**.
...
transparent alternative that hands back `K` with no signal is rejected because it
...
substrate is confused." The additive marker keeps identity changes caller-visible.

**Authorization (required).** Redirect resolution canonicalizes the requested id to the effective
target before the authorization decision. The Gate evaluates `K`, while the audit record preserves
both requested id `C` and effective id `K`. A denied effective target denies the read; the resolver
never returns the tombstone as a fallback. The redirect feature cannot ship unless the Gate input
and audit event carry both identifiers and regression tests prove effective-target authorization.
...
- **Rejects a natural-key collision (fail-loud) rather than dropping the edge.** If the move would
  land the edge on an already-existing `(source_id, target_id, relation)` triple, which is the
  unique natural key, the primitive returns an error and mutates nothing. This is the deliberate departure
...
UUID; `C` keeps its UUID untouched, and a split never tombstones `C`), then `move_edge_endpoint` for
each edge whose semantics belong to `D`. Edge ids and their annotations survive throughout.

**Primitive selection.** Expose the id-preserving endpoint-move primitive. A higher-level split
operation that deletes and recreates edges while rewriting every `annotates` target is rejected:
it changes more records, must discover every annotator, and provides no benefit over preserving the
edge identifier in place.
...
- **Transparent redirect with no marker.** Silently changes which entity a caller's id denotes;
  the caller cannot tell a stale id from a substrate error.
...
- The response shape of `get`/`resolve` grows an optional `redirected_from` field.
- A new curation primitive expands the write surface and its endpoint-validation obligations.
- Redirect resolution authorizes on effective target `K`, so the Gate and audit contracts carry
  both the requested and effective identifiers.
...
- Rewrite of opaque UUID mentions inside note or other free-text bodies: owned by the
