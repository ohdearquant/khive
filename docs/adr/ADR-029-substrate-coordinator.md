...
`"corpus"`, `"archive"`, ...) in `khive.toml`. ADR-031 introduces multi-engine embedding
...
query="X")`, the ADR-013 contract is "search all notes." If notes span `main.db` and
`corpus.db`, the search must touch both backends and merge results.

**P2: Edges may cross backends.** An entity on `main.db` should be able to
link to a document on `corpus.db`. Hard-disallowing this defeats the
unified-graph model.
...
Backends are isolation boundaries, not relevance signals. A deployment's `main` and `corpus`
...
`NamespaceScope` is sealed: it is constructed only by the coordinator from a verified
`&NamespaceToken`, never from a string. The coordinator does not provide an administrative
cross-namespace bypass. Wildcards such as `None`, `"*"`, or `Option<&str>` are not
overloaded for cross-namespace scope.
...
**Substrate vs. granular**: only substrate-kind search federates. Granular pack-defined
kinds are pack-owned per ADR-017 and stay pack-local: they go to whichever backend the
owning pack is assigned to.
...
| Observability metric (main→corpus edge count) | requires full edge scan | SQL aggregate                     |
...
Writes are different: a successful `link(main_entity, corpus_document)` with corpus unreachable
...
Disallow cross-backend operations. `link(a_on_main, b_on_corpus)` returns
...
Any operator running multiple SQLite files across mounts can encounter it.
