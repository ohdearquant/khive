...
khive's foundational substrate (ADR-001 through ADR-016) is closed and stable. Eight
base `EntityKind` variants plus the KG `resource` kind (ADR-001), five base note kinds
(ADR-013), 17 edge relations (ADR-002 and ADR-055), and eight storage capabilities
(ADR-005) form the foundation.

But foundation is not the entire ecosystem. Deployments may need additional note or
entity kinds, lifecycle rules, provenance edges, or query behavior.
...
`Visibility::Subhandler` is internal to pack dispatch and is not registered as a top-level
MCP verb.
...
Packs that derive state from the event log
...
let mut state = tx.load_state(consumer_id).await?;
...
tx.save_state(consumer_id, &state).await?;
tx.save_cursor(consumer_id, &EventCursor::from(&view.event)).await?;
tx.commit().await?;
```
...
1. Logs the error with `(pack_id, consumer_id, event_cursor)`.
...
declare ownership of the same granular kind, the
...
When two or more pack instances declare the same granular kind, the registry preserves all owners:
...
- **Writes route deterministically** to `primary_write_instance`. The operator MUST
  ...
  a valid tiebreaker.
- **Explicit override** is supported via an explicit `instance=` argument on writes
  (subject to auth).
...
The default `REQUIRES = &[]` covers packs with self-contained vocabulary (such as kg).
...
The 17 edge relations (ADR-002 and ADR-055) are closed. Their semantics are universal: `depends_on`
...
```rust
// Example: a pack declaring theorem -[depends_on]-> definition (both concept subtypes)
...
Per ADR-015, packs also declare a `SchemaPlan` for auxiliary tables. Pack schemas use
`CREATE TABLE IF NOT EXISTS` and are applied at boot to the pack's assigned backend.
...
The default distribution ships the KG pack only. Its registered 19-verb catalog is
exhaustive for this distribution:

| Pack   | Kinds                         | Verbs                                                                                                                                                                                       | Edge rules                                              | Purpose                                                                                                            |
| ------ | ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------ |
| **kg** | 5 note kinds + 9 entity kinds | `create`, `get`, `list`, `stats`, `update`, `delete`, `merge`, `search`, `link`, `neighbors`, `traverse`, `context`, `query`, `propose`, `review`, `withdraw`, `resolve`, `whoami`, `verbs` | Base ADR-002 relations plus ADR-055 epistemic relations | Canonical research KG vocabulary, shared CRUD, traversal, discovery, context assembly, and event-sourced proposals |

Extensions may be sibling crates implementing `Pack + PackRuntime`. They register through
the same `VerbRegistryBuilder` mechanism. The registry does not special-case the KG pack.
...
For the primary use cases, compile-time composition
...
Without hooks, every pack that adds a kind must also reimplement CRUD through
parallel handlers that mostly duplicate kg's logic.
...
The 17 edge relations are universal vocabulary: `depends_on` means the same thing in
...
Endpoints may be pack-specific. Per-relation endpoint rules
...
| Per-pack CRUD (every pack reimplements create/list/etc.)               | Duplicates shared behavior and prevents coherent cross-pack composition.         |
...
- `khive-types` stays foundational and `no_std`-compatible. Pack declarations are
  const data.
- The KG pack is not privileged. It uses the same composition machinery available to
