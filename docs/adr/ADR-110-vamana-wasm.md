...
ignores that optional sidecar; only the wasm layer or a future consumer uses it.
...
ANN bridge owns the existing `external_ids.bin` contract, whose
...
with the ANN bridge is out of scope for v0 and requires a future joint
