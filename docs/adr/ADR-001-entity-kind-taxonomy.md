...
people, tools, running services, and generated checkpoints.
...
Checkpoints and snapshots are generated, versioned state artifacts: they pass all five
tests against existing kinds. Running inference engines and deployed APIs are
...
### 8 base entity kinds plus KG `resource`

`khive_types::EntityKind` remains a closed Rust enum with 8 base variants. The KG pack
validator additionally accepts `resource` as a pack-side kind. A resource is actionable
content consumed by agents, including atoms, domains, skills, tools, and runbooks. It is a
public KG taxonomy value even though it is not yet a variant of the shared Rust enum.
...
| **Artifact** | Generated/versioned state: checkpoints, snapshots, embedding indexes            |
| **Service**  | Running operational instances: inference engines, deployed APIs, MCP servers    |
| **Resource** | Actionable content consumed by agents: atoms, domains, skills, tools, runbooks  |
...
| **Artifact** | `checkpoint`, `snapshot`, `export`, `embedding_index`, `state_bundle`                                                                                 |
...
## Entity Classification Semantics

### Classification precedence

Classification follows the precedence below when an entity exhibits signals from more than
one kind. The first matching row determines the base kind. `Concept` is the residual kind
when none of the preceding rows applies.

| Precedence | Kind       | Defining semantics                                                                                          |
| ---------: | ---------- | ----------------------------------------------------------------------------------------------------------- |
|          1 | `Person`   | An individual human.                                                                                        |
|          2 | `Org`      | An organization, laboratory, company, institution, consortium, or standards body.                           |
|          3 | `Document` | An authored communicative work such as a paper, report, book, specification, or thesis.                     |
|          4 | `Dataset`  | A curated collection of records for training, evaluation, or benchmarking.                                  |
|          5 | `Service`  | A running operational instance with an endpoint, health state, or deployment state.                         |
|          6 | `Project`  | A codebase, library, framework, tool, application, or repository.                                           |
|          7 | `Artifact` | Generated, versioned, or materialized state such as a checkpoint, snapshot, index, export, or state bundle. |
|          8 | `Concept`  | An abstract idea, method, algorithm, theory, architecture, research gap, or metric.                         |
...
Checkpoints, snapshots, and embedding indexes fail the 5-test for existing kinds:
...
- "Is flash attention an algorithm or a technique?" is a representative ambiguity
  that does not justify separate base kinds.
...
- Ordered classification precedence reduces ambiguous classification.
