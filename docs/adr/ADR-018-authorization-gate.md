...
- **Shared deployments**: one khive process serving multiple authenticated callers
...
pub struct ActorRef {
    pub kind: String,  // "user" | "service" | "anonymous" | custom
    pub id:   String,
}
...
in a `StrictGate { inner: Arc<dyn Gate> }` adapter, but that is not the default.
...
against the `events` table. Operators can answer "how many denies occurred in the last
hour?" or "which verbs did a given caller attempt?" with structured queries.
...
operator's policy file has a syntax error, the gate fails to evaluate, but that is
...
including incident retrospectives and other auditability requirements. SQL/GQL queries over
the `events` table answer those questions directly.
...
The wire surface accepts bare aliases for some verbs, such as `create`, alongside
fully qualified `pack.verb` forms. Authorization policy requires one stable verb
identity regardless of the accepted wire spelling. The dispatch boundary therefore
resolves pack ownership and canonicalizes the verb before constructing `GateRequest`.
