...
3. **One MCP tool, not N.** Adding a tool per verb (or per verb-pack combination)
   creates a discovery burden. The public 19-verb catalog is exposed through one
...
4. **Transport-agnostic.** The same wire format works over MCP stdio today,
   ...
   transport.
5. **Pack-extensible vocabulary.** Loaded packs add verbs. The DSL accepts any
...
Arguments typed as UUID (entity ids, note ids, edge ids, proposal ids) accept
...
mode in v1; transactional batches require a verb-specific atomic contract.
...
and lists the keys actually present in the prior result to aid debugging.
...
Per ADR-003, gate enforcement is part of the `kkernel mcp` dispatch path and is
implemented by the `khive-mcp` library. The gate sits between the parser and the runtime:
...
Operator subcommands run without the MCP gate. `kkernel mcp` parses the same DSL and
installs the configured caller-facing gate before dispatch.

### Future frontends

`khive-request` is the substrate for additional input formats. Candidate
...
Exposing the public 19-verb catalog as separate MCP tools would require 19
schemas and 19 description entries in the `tools/list` response. One tool with
a structured DSL means one schema, one description, and one client-side
discovery surface.
...
| ---------------------------------- | ------------------------------------------------------------------------------------------ |
| Per-verb MCP tools                 | 19 public tools to maintain; larger `tools/list` response; no batch composability.         |
