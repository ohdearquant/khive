...
Symmetric relations (`competes_with`, `composed_with`) always emit `direction: "both"`, matching the handler's existing symmetric-relation behavior. For a `"both"` row the endpoint order is deterministic but carries no assertion semantics: `source_id` is the parent, `target_id` is the neighbor, and a client must not infer stored endpoint order from a `"both"` row. (Source: [context.rs lines 77-116 at 9442ec2](https://github.com/ohdearquant/khive/blob/9442ec2c52290120c5bf4a4c8a1dc771102658dd/crates/khive-pack-kg/src/handlers/context.rs#L77-L116).)
