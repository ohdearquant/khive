# ADR-068: Per-Process Isolation Topology
...
§"Amendment to ADR-007 and ADR-018 for per-process topology" below\
...
(gate actor-identity gap), ADR-053 (ActorStore/SessionStore, actor threading)

---

## Context

This ADR specifies the supported topology for deploying multiple isolated instances of the
khive runtime: one process per isolated actor namespace, each opening a dedicated SQLite file.
No two actor namespaces share a process.
...
This architecture is correct for local single-actor deployments. For multi-actor isolation,
a `TenantGate` implementation plugged into the same seam would be the natural isolation
mechanism in a shared-process model.
...
always receives `ActorRef::anonymous()`, regardless of which actor is making the request.
...
line 268). This warning is advisory only: it fires but produces no startup enforcement. A
...
- SQLite write concurrency is per-file: one `Arc<Mutex<Connection>>` protects the writer
  ...
  separate files per actor, write mutexes are independent: simultaneous actor writes use
  independent serialization points. The WAL-wedge failure class (stale process holds
  SQLite WAL, returns -32000 on reconnect) is per-file and therefore per-actor.

---

## Decision

### One process per actor namespace, one SQLite file per actor

This topology implements **physical isolation**: each isolated actor namespace runs in a
dedicated `khive-mcp` process opening a dedicated SQLite file. No process opens any
other actor's database file. No two isolated actors share a process.

### Process launch contract (normative)

The process supervisor (the process launcher or equivalent) MUST enforce the following
invariants before a process begins serving requests:

1. **Mandatory non-empty `KHIVE_ACTOR`**: every process is launched with `KHIVE_ACTOR`
   set to the actor's canonical identity string (for example, `actor:my-agent`). An empty or
   absent `KHIVE_ACTOR` is a provisioning error; the supervisor rejects the launch.
2. **Mandatory strict actor attribution**: every process is launched with
   ...
   warning-only for non-strict deployments.
   ...
   **Dependency**: this enforcement is provided by PR #220 (branch
   ...
   of this ADR). The isolation guarantee for issues #199 and #200 holds once PR #220
   is merged and the supervisor sets both `KHIVE_REQUIRE_ATTRIBUTED_ACTOR=1` and
   `KHIVE_ACTOR` per this launch contract. On plain `main` today (pre-#220) the env var
...
3. **Unique actor/path pairs**: the supervisor derives the actor id and the SQLite file path
   atomically from its registry. If a `(KHIVE_ACTOR, KHIVE_DB)` pair already exists in
...
4. **DB path from registry only**: the SQLite file path (`KHIVE_DB`, args.rs line 15) is
   derived from the supervisor's registry, not accepted from untrusted caller input.
   Actor DB paths are confined to a dedicated directory (for example, `/var/lib/<app>/actors/`);
   paths outside that directory are rejected before the process is spawned.
5. **No shared-process deployment**: this topology does not support multiple isolated actors
   sharing one khive-mcp process. Any deployment that relaxes items 1-4 is outside this
   ADR's contract and requires the multi-actor single-process extension described in the
   future-seam section.

**Isolation mechanism**: because each actor's process opens a separate file, data-at-rest
isolation is physical, not policy-enforced. Process A has no file descriptor to process B's
SQLite file. There is no gate policy to misconfigure, no RLS rule to miswrite, and no namespace
check to bypass. The blast radius of any single-actor failure (corruption, WAL wedge, disk
exhaustion, process crash) is bounded to that actor's process and file.
...
actor's identity and that `comm.send` stamps the actor's identity as `from_actor` — is
guaranteed by mandatory strict-mode startup via `KHIVE_REQUIRE_ATTRIBUTED_ACTOR=1`. This is
a deployment-layer enforcement: it holds because the supervisor always sets it, not because
any code path structurally prevents a misconfigured boot. The two guarantees are complementary:
physical isolation prevents cross-actor data access regardless of actor configuration;
strict-mode startup prevents the party-line inbox and mis-stamp by refusing to serve a
misconfigured process.

**Actor identity flow under this topology**:

```
[KHIVE_ACTOR=actor:my-agent KHIVE_REQUIRE_ATTRIBUTED_ACTOR=1] khive-mcp process
      |
      v
KHIVE_REQUIRE_ATTRIBUTED_ACTOR=1: enforce_strict_actor_mode() returns Err
      ...
      |
      v
RuntimeConfig.actor_id = Some("actor:my-agent")        (serve.rs config resolution)
      |
      v
VerbRegistry::dispatch (pack.rs:940-942)
      |
      +-- configured_actor = ActorRef::new("actor", "actor:my-agent")
      |
      +-- token carries actor_id = "actor:my-agent"    (NamespaceToken::mint_with_visibility)
      |
      +-- comm.inbox handler.rs:131 caller_actor = "actor:my-agent"
      |   guard at line 160: "actor:my-agent" != "local" -> to_actor filter IS applied
      |
      +-- comm.send handler.rs:73 from_actor = "actor:my-agent"  (correctly attributed)
```

Note that `GateRequest` at pack.rs line 852 continues to carry `ActorRef::anonymous()` under
this topology. The gate does not receive the actor's identity. This is intentional for
v1: the process boundary already provides physical isolation, so the gate's actor field is
irrelevant to actor separation. `AllowAllGate` remains the default and is appropriate for
a per-actor process where there is only one actor per process.

---

## Amendment to ADR-007 and ADR-018 for per-process topology

This section amends ADR-007 Rev 6 and ADR-018 effective with this ADR's acceptance. These
amendments apply to the per-process topology described in this ADR only; the local-deployment
contract of both parent ADRs is unchanged.

### Amendment to ADR-007 Rule 4 (Gate as single enforcement seam)

ADR-007 Rule 4 describes a TenantGate implementation (a custom crate behind the Gate trait)
that validates the caller's authenticated identity and enforces per-namespace access.

**Amendment**: for the per-process topology specified in this ADR, physical process and file
isolation replaces TenantGate as the actor isolation mechanism. Each process serves exactly one
actor namespace, so there is no intra-process namespace to distinguish via gate policy. The gate
continues to receive `ActorRef::anonymous()` (pack.rs line 852) because no per-request
authentication context is threaded at v1, and no gate policy need key on actor identity when
only one actor's data is reachable from the process.

`AllowAllGate` is appropriate for this topology. ADR-018's note that "AllowAllGate is a
footgun in multi-user or networked contexts" and "deployments serving multiple users should configure RegoGate or a
custom backend before serving traffic" (ADR-018 §"AllowAllGate: the default gate") does not
apply to per-actor-process deployments: each process is single-actor, and `AllowAllGate`
within a process that serves only one actor is not a multi-user footgun. The multi-user
concern arises only when one process serves multiple actors, which this topology prohibits
(launch contract item 5 above).

TenantGate actor-to-gate threading (issue #13) is required only if a future deployment model
places multiple actors in a single process (the multi-actor single-process extension in the
future-seam section). For v1 per-actor-process deployments, #13 is not required.
...
of gating is "a deal-breaker." ADR-007 Rule 4 is where a TenantGate (a custom crate behind
the Gate trait) is named as the isolation mechanism for that shared-process model. This ADR
establishes an alternative: per-process physical isolation defers the need for TenantGate to
shared-process scale.

For v1 deployments under this ADR: the authorization seam (ADR-018 Gate trait) is live at
every verb dispatch, `AllowAllGate` is installed, and audit events fire through tracing per
the ADR-018 contract. The change is that the "configure a policy backend before serving
traffic" recommendation for multi-actor deployments is satisfied by the process boundary
...
Under one-process-per-actor, `KHIVE_ACTOR` is a mandatory operator contract and
`KHIVE_REQUIRE_ATTRIBUTED_ACTOR=1` is a mandatory supervisor setting. When
`actor_id = "actor:my-agent"`, `token.actor().id = "actor:my-agent"`, and the
...
OR where the field is absent or NULL (see `crates/khive-storage/src/note.rs` lines 232-234 for
the `EqOrMissing` variant definition). Legacy messages stored without a `to_actor` property are
therefore visible to an actor-scoped inbox query on a database that contains them.

For v1 per-actor deployments, this legacy path is a no-op on fresh per-actor databases: a
freshly provisioned actor DB has no pre-existing messages and therefore no legacy rows without
`to_actor`. The supervisor provisions a new empty SQLite file per actor (via
`StorageBackend::sqlite(path)` on first open), so no legacy rows are present at provisioning
time.

If actor data is migrated or imported from an existing database, a strict equality filter plus
a quarantine pass is required: any imported messages without `to_actor` must be either assigned
the actor's label or quarantined before the process is placed in service. The migration runbook
...
incorrect as implemented. The consequence is that an actor DB that contains legacy messages
without `to_actor` will expose those messages to the actor-scoped inbox queries. For fresh
deployments this is a no-op (no legacy rows). For migrated or imported databases the quarantine
rule already stated above — assign or quarantine all messages lacking `to_actor` before placing
the process in service — is mandatory and sufficient to close this gap.
...
process from serving, all messages are attributed to the actor's identity. The mis-stamp
...
gate policy needs to distinguish actors within the same process because there is only one
actor per process.

Issue #13 becomes relevant only if a future deployment model requires multiple actors to share
a single process. That path requires the amendments described in the future-seam section.

---

## Future seam: multi-actor single-process (conditional, out of v1 scope)

If a deployment ever requires multiple isolated actors in one process, the following extension
is required before that deployment shape is valid:

1. An HTTP transport layer authenticates the incoming request (JWT, API key lookup) and
   extracts the actor's canonical identity.
...
3. `GateRequest::new` at pack.rs line 852 receives the authenticated `ActorRef` instead of
   `ActorRef::anonymous()`.
4. A `TenantGate` implementation uses `GateRequest.actor` to enforce per-actor namespace and
   inbox isolation.
...
6. ADR-007 Rule 2 (by-ID ops are namespace-agnostic) is reviewed in the multi-actor context:
   whether by-ID reads must be gated against the authenticated actor's namespace is a policy
   decision that the current contract does not specify.
...
One Postgres instance serves all actors. Actor isolation is enforced by RLS policies keyed
on the actor's authenticated identity. The Gate enforces at the verb level; RLS enforces at
the row level.
...
- Logical isolation is weaker than physical. A misconfigured RLS policy is a data breach. The
  ...
  provisioning remains a supervisor risk, mitigated by the invariants in the launch contract
  above.
- The ADR-049 daemon model manages warm Vamana ANN indexes in-process. Postgres with pgvector
  ...
  recall (pgvector HNSW is adequate for moderate corpus sizes but does not match Vamana's 0.952
...
Multiple actors share a single process. Authenticated caller identity is threaded from the
...
- Physical isolation from per-process deployment is a stronger guarantee and costs no
  ...
  scale is premature.
- The blast radius is larger: a compromised or crashed shared process affects all actors.

### Namespace-as-actor (explicit namespace= on every verb call)

Require callers to supply `namespace=<actor-id>` on every verb call. The gate enforces that
the authenticated actor's identity matches the supplied namespace. This leverages the existing
fact that namespace does reach the gate (via `GateRequest.namespace`), unlike actor.
...
### Process startup checklist (normative, supervisor must verify before placing a process in service)

1. `KHIVE_ACTOR` is set to a non-empty, non-`"local"` identity string.
2. `KHIVE_REQUIRE_ATTRIBUTED_ACTOR=1` is set in the process environment.
3. The `(KHIVE_ACTOR, KHIVE_DB)` pair is unique across all live processes in the supervisor's
   registry.
4. `KHIVE_DB` resolves to a path inside the designated actor data directory.
5. If the actor DB was migrated or imported: all messages with a missing `to_actor` property
...
checkpoint thread. A WAL-wedge in one actor's process does not affect any other actor.
Cross-actor write parallelism is free because writes go to independent files.
...
5. Deploy a `TenantGate` enforcing per-actor namespace isolation.
6. Migrate per-actor SQLite data to the shared Postgres instance via export/import tooling.
...
a new gate implementation. Step 6 is a one-time migration operation. No actor data is lost:
per-actor SQLite files remain readable as an archival export format.
