...
1. **Verb-stable interface**: agents call verbs (`create`, `link`, `recall`, `search`). The
...
4. **Trust boundary separation**: operator-context operations (sync, migrations, pack
   introspection) and agent-context operations (gated verb dispatch) have different trust
...
Two Rust binaries with distinct trust boundaries, converging to a single binary.

**`kkernel`** — the management binary. Owns:
...
- No access control — assumes operator context

**`khive-mcp`** — the agent-facing MCP adapter. Owns:
...
khive-types          (domain types, no_std, zero deps)
  ...
  └── khive-runtime  (VerbRegistry, validation, operations)
          ...
          ├── khive-pack-gtd    (GTD lifecycle, 5 verbs)
          ├── khive-pack-memory (memory/recall, 2 verbs)
          └── khive-pack-brain  (brain profiles, feedback loop)

kkernel  (admin binary — all packs + coordinator)
  ├── kkernel::coordinator   (SubstrateCoordinator)
  ├── khive-runtime
  ├── khive-request
  └── khive-pack-*

khive-mcp  (agent binary — packs + gate enforcement)
  ├── khive-runtime
  ├── khive-request
  ├── khive-pack-*
...
khive.toml (illustrative — backend count and names are deployment decisions):
  ...
  [packs.memory]  backend = "main"
  [packs.gtd]     backend = "main"
```
...
| Gate enforcement             | `khive-mcp` (agent binary) | `kkernel` (operator binary)             |
...
The two binaries embody different trust contexts:
...
**Right**: External layers call one of the binaries (subprocess) or a transport adapter
that dispatches into `VerbRegistry`.

### 5. Semantic validation in storage

**Wrong**: `khive-db` decides whether `brain_profile` is a valid `entity_type` or whether
...
exclusively the agent binary's responsibility.
...
The MCP server and admin CLI have different trust requirements. An MCP server exposed to
AI agents must gate which verbs are available. An admin CLI used by operators must not
be constrained by agent safety policy. Conflating both in one binary forces either
...
| ----------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| Single binary (khive-mcp does everything) | Conflates operator and agent trust boundaries. Adding admin commands to MCP server either gates them unnecessarily or creates a security footgun. |
...
- Engineers know where code belongs: verb logic → packs, validation → runtime,
  persistence → db, routing → coordinator, admin → kkernel, agent gating → khive-mcp.
...
- CLAUDE.md owns operational workflow (commands, directory map, build instructions).
  ADR-003 owns architectural invariants (boundaries, dispatch model, anti-patterns).
