...
4. Writer drift: different callers invent different vocabularies.

A closed ontology, defined as a fixed set of allowed relations, solves these problems by forcing every edge
into a canonical bucket.

The entity kind taxonomy (ADR-001) defines eight base enum kinds plus the KG `resource`
kind. The edge ontology must define
which `(source_kind, relation, target_kind)` triples are legal for each relation, and provide
enough relations to cover the required query classes without creating
...
| `variant_of`    | variant → original                     | Modified version (optimized method → original method)                                  |
| `introduced_by` | concept/document → document/person/org | First described in (technique → paper); document authorship (paper → author/publisher) |
...
checkpoint_v2 -[derived_from]-> checkpoint_v1
...
- `Person part_of Project`: a person is a member or contributor of a project;
...
Consumer verbs built over `introduced_by` follow the base contract and delegate to the same
