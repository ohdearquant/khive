...
**Lifecycle.** A successful `put` publishes the object first and inserts the index row second, in the same logical operation, with `created_at` set to that put's acceptance time. A `put` for a `ContentRef` already present (global dedup) is a no-op against the index: the existing row and its `created_at` are unchanged. A successful `delete` removes the row. A subsequent `put` of the same content after a `delete` creates a new row with a new `created_at`; the object is not "resurrected" with its prior creation time. The index is read-your-writes consistent with the backend's own completed `put`/`delete` calls; it is not required to be consistent with a concurrent writer's in-flight call.
...
**Failure and repair.** A crash or index-write failure after object publication leaves an unlisted object, and a deduplicated retry preserves the missing row; physical deletion paths other than `delete` — most concretely ADR-111 §8's orphan GC — leave stale rows. Neither divergence fails the object operation itself: `put` and `delete` succeed or fail on the capability-by-hash contract alone. Divergence is healed by reconciliation: an operator-triggered or scheduled sweep compares the index against the object store, inserts missing rows (with the sweep time as `created_at`), and removes rows for objects no longer present. (Source: [ADR-111](ADR-111-blob-store.md) §8 "Orphan GC".)
...
`cursor` is an opaque token encoding the `(created_at, content_ref)` of the last row returned on the prior page, using the canonical `created_at` encoding above. A `cursor` that does not decode to that shape, or that decodes but does not correspond to a value the index could have produced, is rejected with a validation error rather than silently treated as "start of list".
