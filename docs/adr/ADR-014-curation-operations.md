...
at the same key rather than recursively merging. Removing an existing property key is
not supported in v1; a future explicit `unset` mechanism may add that operation. For
`tags`, semantics are
...
2. Verify namespace ownership.
3. Apply patch fields.
4. If kind_status changed: validate transition against the declared NoteKindSpec lifecycle
   (ADR-004).
...
### `content_strategy`: description merge is independent of `policy` (amendment)
...
merge(kind="note", from_id, into_id):
  ...
  2. Validate that both notes have the same note kind
...
After a successful merge, `from_id` has `deleted_at` set to the merge timestamp and
`properties["_merged_into"]` = `into_id`. The record
...
### Hard-delete is explicit

Hard-delete is not the merge default. `merge` produces tombstones. Permanently removing
a tombstoned note requires an explicit `delete(kind="note", id=from_id, hard=true)` call,
subject to the authorization policy in ADR-018.
...
- Cascades to incident edges so no dangling endpoint references remain.
...
explicitly. Storage and query semantics for events are governed by ADR-004 and ADR-022.

### Property unset and edge property updates

`v1` does not support `unset(key)` as a first-class operation, and patch updates
cannot remove an existing key from `properties`. A future extension may add explicit
unset semantics if a concrete use case justifies it.
...
merge(kind="observation", from_id=..., into_id=...)  # granular kind; resolves to note substrate
```
...
- ADR-022: Events Query Surface: event retrieval and audit visibility.
