...
| Workload            | Profile         | Reason                                                  |
| ------------------- | --------------- | ------------------------------------------------------- |
| Core KG             | Hot, shared     | Cross-record linking and interactive queries are common |
| Large corpus        | Cold, dedicated | Large index; separate maintenance cadence               |
| Historical snapshot | Read-only       | Frozen data with an independent backup cadence          |

These cannot be served by a single SQLite file without giving up:

- **Separate VACUUM schedules** — running VACUUM on a large corpus blocks hot reads
...
3. **Cross-pack composition on shared backends.** Two packs assigned to the same backend
   compose directly (records can link directly because they share tables).
...
`RuntimeConfig::db_path` (`./data/main.db` by default). The `kkernel backend`
...
are modeled as multiple named pack instances assigned to different backends.
...
- **Pack extension** (in the pack's `schema_plan`): anything pack-specific —
  pack-prefixed auxiliary tables and indexes.
...
- **Pack-specific tables**: prefixed with the pack name, for example `kg_entity_extensions`.
...
[packs.extension_a] = { backend = "main", ... }
```

Collision policy:

```
SchemaCollision { backend: "main", table: "items", packs: ["extension_a", "extension_b"] }
```
...
backend. **Auto-prefixing** (silently renaming `items` → `extension_a_items`) is rejected: it
...
Pack authors do not know the deployment shape. An operator who wants to place a large corpus on a separate file should not
have to recompile or fork its pack. The TOML assignment is
...
path dedup catches equivalent relative and canonical paths as the same
...
independently, hot/cold data interleave on disk. Large-corpus backends require these controls independently.
...
Each pack hardcodes its backend name. Pros: pack authors cannot be misconfigured.
Cons: removes operator flexibility; small deployments may prefer a shared backend. Rejected.
...
- **Multi-backend** — main + corpus + archive + arbitrary additional backends in one daemon
  process.
- **Per-backend tuning** — cache size, WAL pragma, read-only set per backend.
- **Cold/hot separation** — corpus on its own file with its own VACUUM cadence.
- **Per-domain failure isolation** — corruption in `corpus.db` does not corrupt `main.db`.
...
`[[backends.main]]` entry pointing at `./data/main.db`. Existing data is
...
3. **Pre-existing data migration**. Users with current single-backend deployment have
   data in `./data/main.db`. The default config keeps that file as `main`, so
   migration is automatic. If they later add a corpus backend, existing data stays in
   `main.db`; new corpus writes go to `corpus.db`. Document this clearly; provide
