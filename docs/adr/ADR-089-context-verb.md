...
**Status**: Accepted
**Date**: 2026-07-04
**Depends on**: ADR-002 (edge ontology), ADR-016 (request DSL), ADR-017 (pack standard), ADR-049 (daemon warm state)

## Context

Applications that assemble retrieval context need entity-anchored results: the entities matched
by a query plus their immediate graph neighborhood, assembled under a hard output budget.
Without a composed operation, that requires a caller-side chain of two or more calls
...
selection. Callers that need a budgeted graph neighborhood therefore need the composed form as
product surface, not an internal convenience.
...
| ------------ | -------- | ------- | ------------------------------------------------------------------------------------------------ |
| `query`      | string   | none    | Semantic anchor selection via hybrid search. Does not rank neighbors in v1 (see Amendment below) |
| `entity_ids` | [string] | none    | Explicit anchors (UUID, short prefix, or slug per ADR-046 rules)                                 |
...
`neighbors` verb defaulted to `outgoing`, which could omit relevant incoming edges during
context assembly. A new verb is not bound by the old default. The
`neighbors`/`traverse` default was later corrected to `both` as well, so
...
5. **Budget enforcement.** Assembly appends records in the deterministic order until the
   ...
   of each appended record, using the same serialization the response returns, so the count
...
runtime instrumentation pattern so the serve cost is measurable from day one.
...
3. **A retrieval-layer verb**: context is graph surface over entities, owned by the KG
   pack's vocabulary and edge rules. Placing it elsewhere would add a second cross-layer seam for
   no gain.
...
- ADR-023 surface amendment: the verb catalog gains `context`, and the public verb reference
  documents it.
- Callers can replace a flat search-plus-neighbors chain with one `context` call once shipped.

## Amendment (2026-07-04): neighbor ordering

The implementation establishes that `neighbors_with_query` (the runtime op behind both the
