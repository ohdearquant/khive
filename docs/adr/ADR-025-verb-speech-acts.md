...
Theory sense (Austin 1962, Searle 1969). Each verb constitutes an institutional fact
in the namespace: `create` commits the caller to a persistent record, while `review`
declares an approval or rejection.
...
| --------------- | ---------------------------------------------- | ------------------------------------------------------------------------------------------------- | ----------------------------------------------- |
| **Assertive**   | Speaker represents a state of affairs          | `get`, `list`, `search`, `neighbors`, `traverse`, `query`, `stats`, `verbs`, `context`, `resolve` | Retrieves and presents facts from the substrate |
| **Directive**   | Speaker attempts to get hearer to do something | _(none)_                                                                                          | No public KG verb currently uses this force     |
| **Commissive**  | Speaker commits to a future course of action   | `create`, `link`, `propose`, `withdraw`                                                           | Commits the caller to a persistent change       |
| **Declaration** | Speaker brings about a state of affairs        | `update`, `delete`, `merge`, `review`                                                             | Changes institutional status by fiat            |
| **Expressive**  | Speaker expresses psychological state          | _(none)_                                                                                          | No verb currently uses this force               |
...
1. **It introduces a force not redundant with an existing verb in the same category.**
   Adding a first directive requires justification for why it belongs on the
   substrate surface. Adding a first expressive would require justification for why
...
2. **It constitutes an institutional fact.** A verb that merely retrieves data without
   ...
   `search`/`list`/`query`. A verb that changes state is either commissive (caller-
...
force is _inherited_ from the verbs it contains. `request("[create(...), review(...)]")` performs a commissive followed by a
declaration. The request verb itself is a speech-act
...
const HANDLERS: &[HandlerDef] = &[
    ...
    HandlerDef {
        ...
        visibility:  Visibility::Verb,
        description: "Approve or reject an open proposal.",
        // ...
    },
];
...
caught by implementation checks, not by the type system. The point is naming the classification so
...
yet." (The product surface has grown: the kg pack now carries 18 verbs including `propose`,
`review`, `withdraw` from ADR-046 and `verbs` discovery; the full classified surface is in the table above.) The first verb-addition pressure (e.g., for a future audit/compliance pack) will
...
- **Edge cases**: some verbs straddle categories. `propose` is primarily commissive
  because it creates a persistent proposal, while it also invites a later review. The
