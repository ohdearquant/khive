...
> Relationship to ADR-030: ADR-012 governs high-level runtime and pack orchestration of storage
> capability signals, `NamespaceToken` enforcement, fusion, filtering, and alive
> checks. ADR-030 assigns low-level engines and adapters to `khive-retrieval`.
> The two decisions are complementary layers of the shipped retrieval path.

## Context

khive retrieves entities, notes, and graph subsets from typed, multi-substrate storage.
A caller might ask "find me FlashAttention" (vector + text), "what depends on
HNSW?" (graph), "decisions about quantization from last month" (note + temporal filter),
...
2. **Method neutrality.** Built-in fusion strategies (RRF, weighted) coexist with
   custom pack-defined and learned strategies.
...
ranked result set. The composition layer lives in `khive-runtime` and
...
3. **Graph expansion**: given a seed set, walks N hops via `GraphStore` to expand or
   ...
   from the 17 canonical `EdgeRelation` values (ADR-002).
...
5. **Reranking**: optional post-fusion pass. v1 ships `rerank` (cosine reranking
   over a candidate subset). ADR-042 specifies the composable cross-encoder path.
...
`Custom` is the openness mechanism. Packs may register named strategies that weight candidates
using domain-specific signals.
...
- **ADR-042**: reranker traits, local rerank configuration, and in-process inference integration.
...
layer. Retrieval strategies (RRF, Weighted, Custom, calibration-weighted, etc.) are **fusion
...
a calibration-weighted strategy does not change the backend. The two axes evolve independently.
...
1. **GNN as embedding source**: produces graph-context-aware vectors. Consumed via
   `VectorStore` like any other embedding. An embedding provider may add a GNN variant.
...
both `main` and `lore` SQLite files) is the SubstrateCoordinator's job (ADR-003). The coordinator:
...
| Multi-modal retrieval (image, audio)              | Requires an embedding provider with non-text support.                    |
...
cases. They do not cover every domain-specific or learned strategy. Without a `Custom` variant, packs
...
Graph traversal is retrieval. A caller asking "what does FlashAttention depend
...
### Why a standalone `khive-retrieval` crate?

ADR-030 assigns reusable HNSW, BM25, hybrid-search, and fusion primitives to the
dedicated `khive-retrieval` crate. This isolates a distinct algorithmic concern with
its own tests, benchmarks, and dependency surface. ADR-012 remains the composition
layer above `khive-retrieval`, not the owner of retrieval primitives.
...
| Keep all retrieval primitives in `khive-runtime`               | Reusable engines and fusion primitives warrant a dedicated crate.       |
...
- Pack-defined and learned retrieval methods are first-class peers.
...
- Cross-backend fan-out adds coordinator orchestration.
  Mitigated: single-backend deployments avoid fan-out overhead.
...
- ADR-013: Note Kind Taxonomy: supersession filter for note retrieval.
