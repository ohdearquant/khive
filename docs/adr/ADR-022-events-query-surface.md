...
### 3a. EventFilter predicate contract

`EventFilter` is the canonical v1 predicate type. The storage layer lowers it to
SQL, and replay consumers may apply the same field semantics in memory.
...
- `kinds`: typed event discriminants. Use when consumers handle specific `EventKind`
  variants (projections, workers, replay).
...
**Canonical replay order** (for deterministic
catch-up):
