...
2. **`search_with_filter`** — metadata predicate pushed into the vector index scan.
   ...
   every filtered query, including namespace-scoped retrieval.
...
not enabled in v1). Capability advertisements MUST NOT include `Hnsw`
...
pub enum VectorIndexKind {
    /// Reserved discriminant for a future HNSW backend (ADR-005 §rationale).
...
cosine, not HNSW. `Hnsw` is reserved for a future HNSW backend (ADR-005 §rationale).
...
tables (`entities`, `notes`). The full SQL shape is:
...
`Range` for filtered retrieval and namespace multi-select.
...
collect all candidate sets and fuse even when some queries fail (degraded retrieval is better
than no retrieval). The caller decides whether to propagate, log, or ignore inner errors.
...
**Consumers:** ADR-043 `EmbedMigrationWorker` uses `update` for incremental re-embed. Future
re-embedding flows use the same method.
...
/// substrate (`entities` / `notes` with `deleted_at IS NULL`). A note vector is protected
/// by its corresponding live row in `notes`.
///
...
| Consumer                                   | Method                           |
| ------------------------------------------ | -------------------------------- |
| Namespace/kind-gated retrieval             | `search_with_filter`             |
...
ADR-043 will be amended to cite these methods explicitly.
...
- `candidate_multiplier` drops to 2–3 for filtered queries (from 20+ with
  post-hoc filtering).
...
whether field-disambiguated retrieval is available without a runtime probe.
