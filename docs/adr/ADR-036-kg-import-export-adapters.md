...
The deferred mapping-file design in §3 will allow explicit column-to-field mapping. It is not
part of the shipped CSV/TSV path.
...
JSON key-path mapping is deferred with the mapping-file design in §3.
...
The shipped conflict flags operate independently of the deferred mapping and schema-mode
designs. Until those designs are implemented, combining `--mapping` or `--schema-mode` with
any import mode is rejected before import begins.
...
khive kg import: CSV -> NDJSON -> working.db
  ...
  entities:  1,203 imported, 0 skipped, 0 errors
  edges:     389 imported, 12 skipped, 0 errors
...
- JSON: **P0 exception: eager parse.** The P0 `JsonFormatAdapter` uses
  ...
  starts. P1 replaces this with
  `serde_json::Deserializer::from_reader` streaming
...
| `--default-kind` | entity kind string       | none                                        | Kind assigned when source has no kind                                                         |
...
| `--output-dir` | dir path | none    | Required for `--format markdown` (P2) |
...
existing [ADR-020](./ADR-020-git-native-kg-implementation.md) import path, while adapters are thin
...
### 4. Deferred schema-inference transaction boundary

If schema inference is implemented, schema expansion will be a deliberate act: the user chooses
`--schema-mode infer` knowing that new
...
- Researchers can import shipped CSV, TSV, and JSON inputs without a separate preprocessing
  pipeline.
...
- The adapter boundary permits additional formats without duplicating import validation.

### Negative

- Each shipped format parser must handle dialect and encoding variation found in real files.
- JSON import currently buffers its input before iteration; large arrays increase peak memory.
- Activating the deferred RDF, Markdown, or schema-inference designs will require the additional
  buffering and semantic safeguards described in their sections.
...
- If mapping files are implemented, an explicit mapping will replace auto-detection rather than
