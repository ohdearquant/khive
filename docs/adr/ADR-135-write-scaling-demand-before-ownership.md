...
Kill condition: priority scheduling is removed in favor of FIFO if adversarial tests show ordering-key violations, starvation of admitted bulk work beyond its SLO, or interactive p99 improvement below 10% at the cost of more than a 10% reduction in useful throughput. Conversely, durable handles become required if more than 0.1% of admitted writes outlive caller deadlines or any accepted write can survive owner restart without a recoverable caller outcome.
...
- `writer_queue_admission_rejected` or `writer_queue_admission_timeout`: the request was not accepted;
