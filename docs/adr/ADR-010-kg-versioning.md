...
1. **Automation-safe.** Graph writers need durable, diffable, replayable history.
   Versioning must work for both automated and interactive callers.
2. **Reviewable.** Reviewers need familiar inspection workflows. `git diff`, pull
   requests, and code review tools should work on KG data.
...
Graph changes are serialized deterministically, stored in git, and reviewed through
standard git tooling. khive adds graph-aware validation, diffing, and conflict reporting
without defining a separate collaboration platform.
...
v1 snapshots cover entities and edges only. Notes and events are excluded.
...
NDJSON files in a git repository provide a shared representation that callers and reviewers
can read, write, and diff. The `kkernel sync` step rebuilds the queryable database from the
canonical NDJSON source.

### Why entities + edges only in v1?

Notes carry temporal, cognitive, and potentially private content (observations and decisions).
Versioning notes requires export/import format decisions, privacy/redaction
...
### Why existing git hosting for collaboration?
...
- Automated callers and reviewers share the same representation (NDJSON in git).
