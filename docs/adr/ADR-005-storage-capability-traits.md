...
The trait does not enumerate retrieval methods. Different backends and subsystems
compute relevance from multi-vector records in different ways. ColBERT-style late
interaction, max-sim aggregation, mean pooling, per-section reranking, and learned
combinations are all valid. The `VectorSearchRequest`
...
### VectorStore and index engines

`VectorStore` is the khive-facing contract. The concrete vector index engine is an
implementation detail inside `khive-db`. Callers depend on `Arc<dyn VectorStore>`, not
on any engine's concrete API.

`khive-storage` does not depend on engine or scoring types. `VectorSearchHit` carries
scores as `i64` (the raw `DeterministicScore` representation): the storage layer does
not import the `DeterministicScore` newtype from `khive-score`. The runtime converts
between the raw i64 and `DeterministicScore` at the boundary.
...
`FilterExpression` to its engine's native filter representation.
...
(aggregation strategy name, normalization, search budget, learned parameters, etc.).
The trait does not standardize what hints mean across backends: that is each
backend's contract with its callers, documented in the backend crate, not in the
trait surface. An HNSW or multi-vector (ColBERT-style) engine, khive-db's brute-force
vector index, khive's hand-rolled retrieval pipelines, and future backends each define
their own hint vocabularies.

A future engine-integration ADR may replace `VectorStore` with a thinner `VectorIndex`
trait if an external engine's public API stabilizes. The hint mechanism is the seam that
makes this possible without locking khive into any engine's aggregation taxonomy.
