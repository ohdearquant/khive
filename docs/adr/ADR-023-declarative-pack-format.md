...
| `db_diagnostics` | assertive   | Writer-contention and WAL/checkpoint operator diagnostics (ADR-091/ADR-135): finite-wait main-pool writer acquisitions/timeouts, swallowed best-effort audit append failures, build identity, checkpoint counters, a single PASSIVE checkpoint probe, WAL sidecar file size, and an explicitly qualified WAL-pin holder census. The probe may backfill WAL frames (normal checkpoint I/O) but never changes logical state, escalates to TRUNCATE, creates a missing database file, or deletes sidecar evidence. |
