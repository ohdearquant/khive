...
bare verb names. This document deliberately makes no total normative. The authoritative set is
the 20-entry `KG_HANDLERS` table in `crates/khive-pack-kg/src/handler_defs.rs`, cross-checked by
`KG_SUBSTRATE_VERBS` in `crates/kkernel/tests/verb_namespace_contract.rs`; the current surface is
readable at runtime with `request(ops="verbs(pack=\"kg\")")`. The table below classifies the
shipped semantic allowlist by speech act, which is this section's actual subject:
...
| `db_diagnostics` | assertive   | Reader/writer-contention, graph-edge integrity, and WAL/checkpoint operator diagnostics (ADR-091/ADR-135/ADR-165): reader admission capacity/availability, pooled/standalone route counters, admission timeouts and hold lifecycle; aggregate plus pooled/standalone/writer-task writer acquisitions, finite-wait pool-mutex timeouts, swallowed best-effort audit append failures, build identity, graph-edge duplicate-ID and list-ledger counts, checkpoint counters, a PASSIVE probe, WAL size, and a qualified holder census. The probe may backfill WAL frames but never changes logical state, increments write-traffic acquisitions, escalates to TRUNCATE, creates a missing database file, or deletes sidecar evidence. |

`db_diagnostics` shipped 2026-08-01 as the twentieth kg-substrate verb, present in both
`KG_HANDLERS` and `KG_SUBSTRATE_VERBS`. Earlier revisions of this ADR listed it as a
future-design name to be excluded from client allowlists; that exclusion is lifted.
Its 2026-08-08 integrity extension adds `graph_edge_integrity` (duplicate edge-ID groups plus
...
## Proposed amendment (2026-09-15): diagnostics process identity

**Status: Proposed.** Addresses [#2346](https://github.com/ohdearquant/khive/issues/2346).
...
seconds (UTC), or `null` with `process.started_at_unavailable_reason` when unavailable. It is not
pool creation or first-request time. Checkpoint counters are process-global, while reader and
writer acquisition/task counters can reset when the main pool is reconstructed within the same
process.
