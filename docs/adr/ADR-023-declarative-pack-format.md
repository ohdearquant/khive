...
the 19-entry `KG_HANDLERS` table in `crates/khive-pack-kg/src/handler_defs.rs`, cross-checked by
...
`db_diagnostics` remains a future-design name, not a shipped handler. It is absent from both
`KG_HANDLERS` and `KG_SUBSTRATE_VERBS`, so it must not appear in the current catalog or in a
client allowlist derived from this table.
