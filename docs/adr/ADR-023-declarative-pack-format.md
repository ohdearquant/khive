...
- All built-in and third-party packs are Rust crates implementing that trait.
...
1. The **callable verb surface**: what an MCP caller sees through `request("pack.verb(...)")`.
...
In scope: MCP-facing verb surface, handler visibility, composition semantics, naming
...
pub enum Visibility {
    /// Exposed to MCP callers through `request("pack.verb(args)")`.
    Verb,
    /// Not on the MCP wire. Callable only via `kkernel exec '<pack>.<handler>(args)'`.
    /// Addressable through the operator CLI but not registered as a top-level MCP verb.
    Subhandler,
}
```

The MCP transport filters by `visibility == Verb` when building the capability
list. `Subhandler` handlers are unreachable from MCP.

The kkernel CLI exposes the **full** handler surface through the verb-DSL `exec`
subcommand. MCP exposes only handlers marked `Visibility::Verb`.
...
verb entry in each pack crate, and mark
...
1. Pack authors set each handler's static `Visibility::{Verb, Subhandler}` in
   `Pack::HANDLERS`.
2. The public distribution loads the KG pack. Builds that include third-party extensions
   may select among the packs linked into that binary as described by ADR-027.
...
bare verb names. The registered `KG_HANDLERS` table contains the following 18 verbs:
...
`verbs` provides a machine-readable discovery endpoint. It is a pure read operation with
no side effects. It excludes internal subhandlers
...
Every other pack prefixes its verbs with the pack name and a single dot. Sub-variants
use snake_case within the pack namespace, not nested dots.
...
- A bare name (no dot) must be in the KG-substrate allowlist shown above.
...
```
✓ search(entity_type="paper", limit=10)
✗ search(ent_ty="paper", limit=10)

✓ link(source_id="...", target_id="...", relation="extends")
✗ link(src="...", dst="...", relation="extends")
```

Full-word field names retain their meaning in JSON payloads, audit logs, and error
frames after the verb-call context has been lost.
...
This keeps verb semantics predictable for callers: no surprise behavior swaps mid-call.
...
**No macros, no DSLs**: the generated crate is plain Rust. Developers can read and
modify the result directly; rust-analyzer works without expansion magic;
...
- The npm distribution model (ADR-026) supports binaries that include selected
  third-party crates. Users who need such extensions build a binary with those crates
  linked.
...
YAML manifest cannot declare a verb handler: only the vocabulary. Vocabulary-only
extension is insufficient when an extension also requires behavior, so the YAML path
would create a second concept and validation route to maintain.
...
Visibility answers one transport question: whether a handler is on the MCP wire. Usage
guidance belongs in API and operator documentation and does not alter transport exposure.
A handler that must not be remotely callable is a `Subhandler`; a callable handler remains
a `Verb` even when it is rarely used.

### Why no overrides / no middleware?

**Predictability for callers.** If another pack could override kg's `create` verb,
the call could behave differently depending on which packs are loaded in a deployment. Callers would have to introspect the pack set before reasoning about
verb semantics.
...
packs real extensibility without breaking the caller's mental model.
...
1. **JSON and log portability.** Field names travel into error frames and audit logs,
   where the verb-call context may be absent. `ent_ty: "paper"` in an error frame is harder to debug than
   `entity_type: "paper"`.
...
3. **Consistent API vocabulary.** Full-word fields avoid a separate abbreviation policy
   and make generated schemas self-explanatory.
...
- **Predictable verb surface for callers.** Bare verbs ⇒ kg. Dotted ⇒ pack-prefixed.
...
- **Auditable visibility.** The full handler surface is available to operator
  ...
  `Visibility::Verb`.
- **No magic.** Plain Rust packs. Rust-analyzer works, stack traces are direct, and
  implementations can adapt the reference crate.
...
- **No runtime pack install.** Third-party packs require a kkernel rebuild. npm
  ...
  the audience (research and automation platforms).
...
| Pack verb override (last-loaded-wins or explicit declaration) | Breaks caller predictability. Verb semantics become deployment-dependent. KindHook is the safe extension point.                                                                                          |
...
| Proc macro (`#[derive(Pack)]`)                                | Adds expansion magic; rust-analyzer and debuggers have to expand it. Cargo-generate produces plain Rust that developers read directly.                                                                   |
...
1. New packs express CRUD-shaped operations (create/read/update/delete over an entity
   ...
   verbs for the same operation, unless the operation carries genuine non-CRUD domain logic (a state machine, an
   atomic multi-step guard, or side effects beyond storing a record) that
   dispatch-by-kind cannot express. Existing named verbs accepted for that reason are
   unaffected; this rule governs new packs and new verbs going forward.
...
field-validation registry analogous to `EDGE_RULES`), and the accompanying implementation alignment.
