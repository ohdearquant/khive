...
| `db_diagnostics` | assertive   | Writer-contention and WAL/checkpoint operator diagnostics (ADR-091/ADR-135): aggregate plus pooled/standalone/writer-task acquisition counters, finite-wait pool-mutex timeouts, swallowed best-effort audit append failures, build identity, checkpoint counters, a single PASSIVE checkpoint probe, WAL sidecar file size, and an explicitly qualified WAL-pin holder census. The probe may backfill WAL frames (normal checkpoint I/O) but never changes logical state, increments write-traffic acquisitions, escalates to TRUNCATE, creates a missing database file, or deletes sidecar evidence. |
