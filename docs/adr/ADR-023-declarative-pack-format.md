...
| `db_diagnostics` | assertive   | WAL/checkpoint operator diagnostics (ADR-091): build identity, checkpoint counters, a single PASSIVE checkpoint probe, WAL sidecar file size, and a WAL-pin holder census. The probe may backfill WAL frames (normal checkpoint I/O) but never changes logical state, escalates to TRUNCATE, creates a missing database file, or deletes sidecar evidence.                                      |
