...
| `backend list` / `info`              | no                                     | Enumerate configured backends                                                           |
...
`engine list`/`status`, `vector capabilities`, `db check`, and `backend list`/`info` never write.
Everything else can.

### Shared conventions

- **Output**: every subcommand prints one line of JSON to stdout by default; most accept
  `--human` for a readable table/summary instead. Logs (tracing) always go to stderr, so piping
  stdout never mixes log noise into the JSON (`main.rs:449-461`).
...
- `kg import <source> --db <path> [--namespace local] [--format archive|json|ndjson] [--verbose]`
  ...
  - A malformed record anywhere in a `json`/`ndjson` array aborts the entire import before any DB
    ...
    (`archive.rs:823-891`).
  - `--source` points at an arbitrary file the operator names. It is **not** the same as reading
...
namespace** (`filter_unembedded`, `reindex.rs:326-338`) — if the vector store can't answer an
existence check, it conservatively assumes nothing is embedded yet and re-embeds everything.
