...
exposing one tool — `request` — that dispatches 81 verbs across 11 packs.
...
`session`, `code`, `workspace`, `blob` — `code` contributes the `code.ingest` L1/L1.5
source-ingest verb plus a `finding` note kind whose `findings.json` batch ingestion is reached
only through the `kkernel code-ingest` admin CLI; `workspace` contributes entity/endpoint
vocabulary, no verbs; `blob` contributes `blob.put`/`blob.get`/`blob.stat` over the `BlobStore`
CAS trait, ADR-111) — this plugin currently ships pattern skills for the first seven; `session`,
`code`, `workspace`, and `blob` have no skill yet (see the Pattern skills table above). Git
provenance ingestion and write verbs (`git.digest`, `git.commit`, `git.branch`, `git.push`) are
provided by a commercially licensed extension and are not part of this distribution.
