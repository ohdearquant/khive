...
exposing one tool — `request` — that dispatches 139 verbs across 14 packs.
...
`git.digest` verb, and four write verbs — `git.commit`/`git.branch`/`git.update_ref`/`git.push` — that shell to
