...
Use `inspect` to observe the brain pack's current state without modifying it. Start here when recall behavior feels off, when onboarding to a new namespace, or when debugging adaptive tuning.
...
The response includes `id`, `lifecycle`, `consumer_kind`, `total_events`, and `exploration_epoch` for each profile.
...
The `state_snapshot` field contains the current posterior means and variances for each weight parameter.
...
A `NotFound` error means no profile is bound for that combination — the default wildcard binding may be missing or the pack may need a reset.
...
Use the inspect skill to gather facts. When inspection reveals a problem (e.g. wrong lifecycle, unexpected posterior state, missing binding), switch to the `tune`, `manage`, or `bind` skill to act.
