...
# Step 2: link new → old with supersedes
link(source_id=new.id, target_id="<old-note-id>", relation="supersedes")
```
...
Old note is now excluded from `search(kind="note")` results automatically. It's preserved for
history via `get(id=old_note_id)`.
