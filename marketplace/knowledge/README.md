...
| `knowledge.edit`         | `id` (required, UUID or slug), `sections` (required, array of `{section_type, content, heading?, sort_order?}`)   | Upsert sections within an atom. Sections are content-addressed by `content_hash`: identical content refreshes metadata in place, while distinct content of the same `section_type` coexists as sibling sections.                                                                                                      |
| `knowledge.list`         | `type?` (`"atom"` or `"domain"`, default `"atom"`), `limit?` (default 20, max 500), `offset?`                     | Paginate atoms or domains.                                                                                                                                                                                                                                                                                            |
| `knowledge.get`          | `id` (required, UUID or slug), `include_sections?` (boolean, default false)                                       | Fetch a single atom or domain by UUID or slug. When `include_sections=true`, the response includes a `sections` array (fields: id, atom_id, namespace, section_type, heading, content, content_hash, status, tokens, sort_order, created_at, updated_at). Short UUID prefix is NOT supported — use full UUID or slug. |
...
| Verb               | Params                                                                                                                                                                                                                                                                                                                            | What it does                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| ------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `knowledge.search` | `query` (required), `type?` (`"atom"` or `"domain"`), `status?`, `exclude_status?`, `include_drafts?` (boolean, default false), `role?`, `limit?` (default 10, max 100), `min_score?`, `weights?`, `decompose?` (boolean), `decompose_threshold?`, `intersection_bonus?`, `rerank?` (default true), `rerank_alpha?` (default 0.7) | Hybrid FTS + embedding search over atoms/domains. **Draft and deprecated atoms are excluded by default.** Pass `include_drafts=true` to include drafts (deprecated always excluded). Use `status=` for an exact status filter (overrides `include_drafts`). Status values: `draft` \| `reviewed` \| `deprecated`. `rerank=true` blends TF-IDF and embedding scores via `rerank_alpha` (0 = pure embedding, 1 = pure TF-IDF). Score bands: `score>=0.46` reliable, `0.42<=score<0.46` mixed, `score<0.42` off-target. |
...
| Verb                | Params                                                                                                                                       | What it does                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| ------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `knowledge.suggest` | `query` (required, **5+ words**), `role?`, `limit?` (default 8)                                                                              | Domain suggestion via FTS + Vamana ANN + embedding rerank. **Draft and deprecated domain atoms are excluded by default** (same quality default as `knowledge.search`). Returns domain IDs, names, and scores. Short queries cause disambiguation — use 5–12 words for best results.                                                                                                                                                                                                                                             |
| `knowledge.compose` | `query` (required), `domain_ids?`, `atom_ids?`, `max_tokens?` (default 8000), `auto_limit?` (default 5), `explain?` (boolean, default false) | Compose a knowledge briefing from domains/atoms with section-level hybrid scoring (ADR-051). **Auto-compose**: omit `domain_ids`/`atom_ids` to run suggest internally (query must be **10+ words**). Explicit IDs accept any non-empty query. **Output**: by default returns the `markdown` briefing plus compact `atoms`/`domains` and `count`. Pass `explain=true` to also return `sections[]` with per-section `breakdown` scores, `section_count`, and inline section scores in the markdown (for ranker tuning/debugging). |
