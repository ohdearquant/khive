...
| `list`      | **kind** (entity\|edge\|note\|event\|proposal); entity_kind, entity_type, note_kind, tags, source_id, target_id, relations, min_weight, max_weight, limit, offset; event: event_kind, event_kinds; message: thread_id, direction, from, to, read. kind="edge" pages the full set at `offset` (capped at 1000/page); for O(1)-at-depth full-graph walks pass `after` (keyset cursor — the last edge `id`, or `""` to start) instead — the response then becomes `{"edges":[...],"next_after":<uuid-or-null>,"warnings"?:[...]}`. Every other kind returns `{"items":[...],"warnings"?:[...]}`. Each kind enforces its own server-side row cap (entity 500, note 200, edge 1000, event 1000); a `limit` above the cap is clamped, and `warnings` names the cap when it actually bound — a `limit` above the cap that matches fewer rows than the cap does not warn (issue #894).                                                                                                                                                                                                                                                                                                                                                    | `{"kind":"entity","entity_kind":"concept","tags":["ml"]}`    |
