...
| `db_diagnostics` | Reader/writer-contention, graph-edge integrity, and WAL/checkpoint operator diagnostics: reader admission/route/timeout/hold evidence, writer acquisition counters, audit failures, duplicate edge-ID and list-ledger counts, PASSIVE probe, WAL size, holder census (probe may backfill WAL frames — normal checkpoint I/O, never TRUNCATE) | Diagnosing reader/writer stalls, legacy edge integrity, or WAL growth          |
