...
### KG pack — 20 verbs (bare names, no prefix)
...
| `db_diagnostics` | Writer-contention plus WAL/checkpoint operator diagnostics: aggregate and class-specific writer acquisition counters, pooled checkout timeouts, audit failures, PASSIVE probe, WAL size, holder census (probe may backfill WAL frames — normal checkpoint I/O, never TRUNCATE) | Diagnosing writer stalls or WAL growth on a live store                  |
...
**Create is create-always unless a note supplies an explicit natural key.** A singleton `create`
(top-level `kind`) writes exactly one new entity or ordinary note and returns its UUID (a request that also supplies
`edges` additionally writes those edge records, and on a note create `annotates` likewise writes
annotation edges from the new note -- entity creates ignore `annotates`). A non-empty note
`external_id` (or identical `properties.external_id`) instead uses the live natural key
`(namespace, note kind, external_id)`: the winner returns `created=true`; retries/races return the
same canonical UUID with `created=false`, `deduplicated=true` and do not replay hooks or edges.
A bulk `create(items=[...])` carries a `kind` per item and may mix entities and notes. Entities
require `name`; ordinary notes require `content`; loaded task hooks accept task fields per item,
while finding accepts `title` (other finding metadata remains in `properties`), and both may derive
content. Record payload fields belong inside each item; `atomic` and
`verbose` are bulk-only controls. Best effort is
the default: one invalid item does not abort its siblings, and the ordered `results` array carries
one `{index, ok, ...}` entry per input alongside `attempted`/`created`/`skipped`/`failed` counts.
Pass `atomic=true` to validate every item before one all-or-nothing mixed row/FTS transaction. An
empty `items=[]` is accepted and writes nothing.
...
change entity write semantics. Pass it when you have already resolved the reference, so the create
does not repeat that retrieval. Notes without `external_id` remain duplicate-tolerant and are not
resolved by name.
...
Function-call batches can combine independent operations and dependent chain groups. Each
comma-delimited group runs in parallel; `|` operations inside a group run sequentially, and `$prev`
is scoped to that group. The JSON op form remains flat and does not support `$prev`:

| Situation                       | Use                                        | Note                                                                                  |
| ------------------------------- | ------------------------------------------ | ------------------------------------------------------------------------------------- |
| Ops are independent             | batch `[a(), b()]`                         | parallel, up to 100 ops                                                               |
| Op B needs op A's result        | chain `a() \| b(…=$prev…)`                 | sequential, aborts on failure                                                         |
| Two independent dependent flows | `[a() \| b(…=$prev…), c() \| d(…=$prev…)]` | groups parallel; each chain aborts independently                                      |
...
An unbracketed mixed form such as `a() | b(), c()` is rejected, as is `$prev` inside the JSON op
form. Put mixed groups inside the function-call batch brackets:
...
The three top-level groups run concurrently; failure in the dependent chain does not abort either
singleton group. The 100-op limit counts every operation across all groups.
...
| Tool             | Fields                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | Example                                                        |
| ---------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------- |
| `create`         | **kind** (entity\|note, or a granular entity/note kind; not edge/event/proposal), **name** + **entity_kind** for entity, **content** + note_kind for an ordinary note; entity_type, description, properties, tags, salience, edges (writes typed edges from the new record, either kind), annotates (note only: annotation edges from the new note), skip_dedup_check (bool, default false: skip the post-create `similar_existing` hybrid-search hint). Loaded task hooks accept task fields; finding accepts `title` while other finding metadata remains in `properties`; both may derive content. Task salience is derived from priority, so task creates reject explicit `salience`. `embedding_content` (singleton note only) overrides the vector-embedding input with a non-empty proper prefix of `content`; stored and FTS-indexed content stay full. A note may supply a non-empty `external_id` (top-level or in `properties`) as an idempotent `(namespace, note kind, external_id)` natural key. Bulk shape: **items** (mixed entity/note specs, each with **kind**; entities require **name**, ordinary notes require **content**, task fields and finding `title` are accepted per item, capped at 1000) in place of a top-level kind. Record payload fields belong inside each item; `atomic`/`verbose` are bulk-only. Best effort is the default (`atomic=false`) and returns one ordered result per item; `atomic=true` makes entity/note rows and FTS documents all-or-nothing. `verbose=true` includes full `entities` and `notes`. Bulk entity vectors remain deferred; newly created notes are indexed after commit.                                                                                                                                                                                                                                                                                                                                                                                                                                      | `{"kind":"entity","entity_kind":"concept","name":"LoRA"}`      |
...
| `db_diagnostics` | No fields. Returns `{build, db_path, wal_file, checkpoint_counters, checkpoint_probe, checkpoint_probe_error, writer_contention, wal_pin}`. `writer_contention.writer_acquisitions` is the total of `pooled_writer_acquisitions`, `standalone_writer_acquisitions`, and `writer_task_acquisitions`; `writer_acquisition_timeouts` is the finite-wait pool-mutex stage, and `audit_append_failures` counts swallowed best-effort audit errors. `wal_pin.status` and tagged `wal_pin.census.status` distinguish degraded, incomplete, and unavailable evidence. The PASSIVE checkpoint probe may backfill WAL frames into the database (normal checkpoint I/O); it never changes logical state, escalates to TRUNCATE, creates a missing database file, increments write-traffic acquisitions, or deletes WAL-pin sidecar evidence.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | `{}`                                                           |
