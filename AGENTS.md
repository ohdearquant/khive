...
| `db_diagnostics` | Writer-contention plus WAL/checkpoint operator diagnostics: pool checkout/audit-failure counters, PASSIVE probe, WAL size, holder census (probe may backfill WAL frames — normal checkpoint I/O, never TRUNCATE) | Diagnosing writer stalls or WAL growth on a live store                  |
