...
All 11 packs load by default. **66 public verbs** across the packs: the `git` pack
...
three verbs, `blob.put` / `blob.get` / `blob.stat` (ADR-111), over a content-addressed
`BlobStore`. Regenerate via `request(ops="verbs()")`
