...
| `db_diagnostics` | Writer-contention, graph-edge integrity, FTS5 segment/maintenance, and WAL/checkpoint operator diagnostics: aggregate and class-specific writer acquisition counters, pooled checkout timeouts, audit failures, duplicate edge-ID and list-ledger counts, bounded one-row FTS structure readings, PASSIVE probe, WAL size, holder census (probe may backfill WAL frames — normal checkpoint I/O, never TRUNCATE) | Diagnosing writer stalls, legacy edge/index integrity, or WAL growth    |
