...
**Create is not an upsert.** A singleton `create` (top-level `kind`) writes exactly one new record
and returns its UUID rather than reusing an existing one; a bulk `create(items=[...])` carries a
...
`resolve` and inspect its per-ref `Resolved`/`Ambiguous`/`NotFound` status (or `search` and inspect
the ranked hits), and create only after `NotFound` or an intentional decision to represent a
distinct entity. Use `merge` only after establishing that two UUIDs denote the same entity.
