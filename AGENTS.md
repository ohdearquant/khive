...
| `db_diagnostics` | Writer-contention, graph-edge integrity, and WAL/checkpoint operator diagnostics: aggregate and class-specific writer acquisition counters, pooled checkout timeouts, audit failures, duplicate edge-ID and list-ledger counts, PASSIVE probe, WAL size, holder census (probe may backfill WAL frames — normal checkpoint I/O, never TRUNCATE) | Diagnosing writer stalls, legacy edge integrity, or WAL growth          |
